...
from bingo_emulator.graphics.triple_play import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            self.scan_all()
        self.delay(name="display", delay=0.1, handler=graphics.triple_play.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh triple_play")
...
#        self.cancel_delayed(name="blink_title")
        graphics.triple_play.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.triple_play.reel1, graphics.triple_play.reel10, graphics.triple_play.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.triple_play.reel1, graphics.triple_play.reel10, graphics.triple_play.reel100)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def find_winner(self, relays, card, corners, name=0):
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if relays == 3:
                if not corners:
                    if card == 1 or self.game.each_card.position < 5:
                    ...
                    elif self.game.each_card.position == 6:
                        if card == 2:
            ...
            if relays == 4:
                if corners:
                    if card == 1:
                        ...
                        if self.game.each_card.position < 5:
                            if self.game.selector.position >= 2:
                                if self.game.card1_replay_counter.position < fiveodds:
                                    self.game.search_index.engage(self.game)
                                    self.card1_replay_step_up(fiveodds - self.game.card1_replay_counter.position)
    ...
    def closed_search_relays(self, rivets):
        ...
        self.pos[29] = {}
        self.pos[30] = {1:1, 9:2, 15:3, 7:4, 18:5}
        self.pos[31] = {23:1, 21:2, 25:3, 22:4, 3:5}
        self.pos[32] = {2:1, 12:2, 16:3, 14:4, 5:5}
        self.pos[33] = {13:1, 20:2, 11:3, 19:4, 24:5}
        self.pos[34] = {17:1, 8:2, 10:3, 6:4, 4:5}
        self.pos[35] = {1:1, 23:2, 2:3, 13:4, 17:5}
        self.pos[36] = {9:1, 21:2, 12:3, 20:4, 8:5}
        self.pos[37] = {15:1, 25:2, 16:3, 11:4, 10:5}
        self.pos[38] = {7:1, 22:2, 14:3, 19:4, 6:5}
        self.pos[39] = {18:1, 3:2, 5:3, 24:4, 4:5}
        self.pos[40] = {1:1, 21:2, 16:3, 19:4, 4:5}
        self.pos[41] = {18:1, 22:2, 16:3, 20:4, 17:5}
        self.pos[42] = {}
        self.pos[43] = {1:1, 18:2, 4:3, 17:4}
        self.pos[44] = {}
        self.pos[45] = {}
        self.pos[46] = {}
        self.pos[47] = {}
        ...
        if rivets == 13:
            corners = True
        if rivets in range(15,29):
            card = 2
        if rivets == 28:
            corners = True
        if rivets in range(30,50):
            card = 3
        if rivets == 43:
            corners = True
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [284,249]
            image = pygame.image.load('triple_play/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [229,275]
            image = pygame.image.load('triple_play/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [347,272]
            image = pygame.image.load('triple_play/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.triple_play.display, param=self)
        else:
    ...
    def features_probability(self):
        ...
        if self.game.each_card.position < 3:
            self.game.each_card.step()
        if self.game.each_card.position >= 3:
            if self.game.cu:
                self.game.each_card.step()
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.triple_play.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.triple_play.display, param=self)
            s -= 1
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.triple_play.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.triple_play.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.triple_play.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.triple_play.display, param=self)
            s -= 1
...
class TriplePlay(procgame.game.BasicGame):
    def __init__(self, machine_type):
        super(TriplePlay, self).__init__(machine_type)
        ...
        #Odds stepper
        self.odds = units.Stepper("odds", 8, 'triple_play')
        ...
        self.card3_replay_counter = units.Stepper("card1_replay_counter", 300)
        ...
        self.spotted = units.Stepper("spotted", 10, "triple_play", "continuous")
...
game = TriplePlay(machine_type='pdb')
game.reset()
game.run_loop()
