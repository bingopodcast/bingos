...
class MulticardBingo(procgame.game.Mode):
    ...
    def features_probability(self):
        ...
        if sd == 30:
            if self.game.corners2.status == False:
                ...
                self.game.sound.play('tilt')
        if sd == 26:
            if self.game.corners3.status == False:
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.triple_play.both_animation([self, start + num])
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.triple_play.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
