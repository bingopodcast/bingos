...
from bingo_emulator.graphics.nevada import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            self.scan_all()
        self.delay(name="display", delay=0.1, handler=graphics.nevada.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh nevada")
        else:
    ...
    def regular_play(self, ball_return=False):
        ...
        if self.game.start.status == True:
            ...
            self.game.diamond_diagonal.disengage()
            self.game.dd_three_as_four.disengage()
            self.game.dd_three_as_five.disengage()
            ...
            self.game.selector.reset()
            self.game.timer.reset()
            self.game.super_card1_three_as_four.disengage()
            self.game.super_card1_three_as_five.disengage()
            self.game.super_card2_three_as_four.disengage()
            self.game.super_card2_three_as_five.disengage()
            self.game.ball_return.reset()
            self.game.ball_count.reset()
            self.game.extra_ball.reset()
            self.game.sound.play_music('motor', -1)
            self.game.before_fourth.engage(self.game)
            self.game.super_card1_three_as_four.engage(self.game)
            self.game.super_card2_three_as_four.engage(self.game)
            self.regular_play()
        ...
        if self.game.selection_feature_relay.status == True:
            if self.game.before_fourth.status == False and self.game.before_fifth.status == False:
                ...
                if self.game.spotted.position == 10:
                    self.game.super_card1_three_as_four.disengage()
                    self.game.super_card2_three_as_four.disengage()
                    self.game.super_card1_three_as_five.engage(self.game)
                    self.game.super_card2_three_as_five.engage(self.game)
        self.delay(name="display", delay=0.1, handler=graphics.nevada.display, param=self)
...
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.nevada.reel1, graphics.nevada.reel10, graphics.nevada.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.nevada.reel1, graphics.nevada.reel10, graphics.nevada.reel100)
                ...
                for number in self.holes:
                    if number == key:
                        ...
                        if self.game.selector.position >= self.card:
                            if s >= 3:
                                self.find_winner(s, self.card, self.corners, self.supercard, self.dd)
                                break
        if self.game.name.position == 6:
            self.find_winner(0, 1, 0, 0, 0, 1)
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)

    def find_winner(self, relays, card, corners, supercard, dd, name=0):
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if relays == 3:
                if dd == 1:
                    if self.game.diamond_diagonal.status == True:
                        if self.game.dd_three_as_four.status == False and self.game.dd_three_as_five.status == False:
                        ...
                        elif self.game.dd_three_as_four.status == True:
                        ...
                        elif self.game.dd_three_as_five.status == True:
                            if self.game.card1_replay_counter.position < fiveodds:
                                self.game.search_index.engage(self.game)
                                self.card1_replay_step_up(fiveodds - self.game.card1_replay_counter.position)
                elif supercard == 1:
                    if self.game.super_card.position >= 4:
                        if self.game.super_card1_three_as_four.status == True:
                            if self.game.card1_replay_counter.position < fourodds:
                                self.game.search_index.engage(self.game)
                                self.card1_replay_step_up(fourodds - self.game.card1_replay_counter.position)
                        else:
                    ...
                    if self.game.super_card.position == 8:
                        if self.game.super_card2_three_as_five.status == True:
                            if self.game.card1_replay_counter.position < fourodds:
                                self.game.search_index.engage(self.game)
                                self.card1_replay_step_up(fourodds - self.game.card1_replay_counter.position)
                        else:
    ...
    def closed_search_relays(self, rivets, c):
        ...
        self.pos[39] = {}
        self.pos[40] = {2:1, 19:2, 15:3, 20:4, 25:5, 21:6, 5:7, 22:8}
        self.pos[41] = {20:1, 25:2, 21:2, 5:3, 22:4, 2:5, 19:6, 15:7, 20:8}
        self.pos[42] = {}
        self.pos[43] = {}
        self.pos[44] = {}
        self.pos[45] = {}
        ...
        if rivets == 40 or rivets == 41:
            dd = True

        return (self.pos[rivets], card, corners, sc, dd)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [284,249]
            image = pygame.image.load('nevada/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [229,275]
            image = pygame.image.load('nevada/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [347,272]
            image = pygame.image.load('nevada/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            ...
            if r2 == 1:
                ...
                if m2 == 1:
                    self.game.diamond_diagonal.engage(self.game)
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.nevada.display, param=self)
        else:
    ...
    def check_mixer4(self):
        ...
        if m4 == 13 or m4 == 16 or m4 == 18 or m4 == 44:
        ...
        if m4 == 3 or m4 == 15 or m4 == 17 or m4 == 23 or m4 == 25 or m4 == 29 or m4 == 36 or m4 == 45:
            self.game.selection_feature_relay.engage(self.game)
        self.delay(name="display", delay=0, handler=graphics.nevada.display, param=self)
        if m4 == 2 or m4 == 26 or m4 == 37 or m4 == 41 or m4 == 35:
            if self.game.dd_three_as_four.status == False:
                self.game.dd_three_as_four.engage(self.game)
        if m4 == 24 or m4 == 34 or m4 == 40 or m4 == 46:
            if self.game.dd_three_as_four.status == True:
                self.game.dd_three_as_four.disengage()
                self.game.dd_three_as_five.engage(self.game)
        if m4 == 4 or m4 == 1 or m4 == 16 or m4 == 22 or m4 == 24 or m4 == 27 or m4 == 31 or m4 == 35 or m4 == 43 or m4 == 44 or m4 == 48:
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.nevada.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.nevada.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.nevada.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.nevada.display, param=self)
            s -= 1
...
class Nevada(procgame.game.BasicGame):
    """ Nevada is the second United game with a standard Bally 25 hole playfield.  It's not _exactly_ the same, but close enough. """
    def __init__(self, machine_type):
        super(Nevada, self).__init__(machine_type)
        ...
        self.reflex1 = units.Stepper("reflex1", 50, "Nevada", "continuous")
        self.reflex2 = units.Stepper("reflex2", 50, "Nevada")
        ...
        self.spotted = units.Stepper("spotted", 10, "nevada", "continuous")
        ...
        self.diamond_diagonal = units.Relay("diamond_diagonal")
        self.dd_three_as_four = units.Relay("dd_three_as_four")
        self.dd_three_as_five = units.Relay("dd_three_as_five")

        self.anti_cheat = units.Relay("anti_cheat")

        self.super_card1_three_as_four = units.Relay("super_card1_three_as_four")
        self.super_card1_three_as_five = units.Relay("super_card1_three_as_five")
        self.super_card2_three_as_four = units.Relay("super_card2_three_as_four")
        self.super_card2_three_as_five = units.Relay("super_card2_three_as_five")

        self.replays = 0
        self.returned = False

    def reset(self):
        super(Nevada, self).reset()
