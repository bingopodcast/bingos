...
from bingo_emulator.graphics.stock_market import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.stock_market.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh stock_market")
        else:
            if self.game.ball_count.position >= 4:
...
#        self.cancel_delayed(name="blink_title")
        graphics.stock_market.display(self, 0)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.stock_market.reel1, graphics.stock_market.reel10, graphics.stock_market.reel100, graphics.stock_market.reel1000)
                self.game.coils.registerDown.pulse()
                number -= 1
                graphics.stock_market.display(self)
    ...
    def replay_step_up(self):
        if self.game.replays < 8999:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.stock_market.reel1, graphics.stock_market.reel10, graphics.stock_market.reel100, graphics.stock_market.reel1000)
...
#        self.cancel_delayed(name="blink_title")
        ...
        for i in range(0, 100):
            ...
            if i >= 51:
                self.r = self.closed_search_relays(self.game.searchdisc2.position + 50, self.game.corners4.status, self.game.corners5.status, self.game.corners6.status)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
        ...
        if self.game.switches.left.is_inactive() and self.game.switches.right.is_inactive():
            if i[0] == 1 and self.game.card1_replay_counter.position > 0:
                ...
                if i[0] == 1 and self.game.card1_missed.status == True:
                    ...
                    graphics.stock_market.blink_double(self)
    ...
    def closed_search_relays(self, rivets, c1, c2, c3):
        ...
        if rivets in range(51,66):
            card = 4
            if rivets == 51:
                corners = True

        if rivets in range(66,80):
            card = 5
            if rivets == 66:
                corners = True
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [30,266]
            image = pygame.image.load('stock_market/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [192,270]
            image = pygame.image.load('stock_market/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [330,272]
            image = pygame.image.load('stock_market/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [524,270]
            image = pygame.image.load('stock_market/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class StockMarket(procgame.game.BasicGame):
    """ Bright Lights was the first bingo produced by Bally """
    def __init__(self, machine_type):
        super(StockMarket, self).__init__(machine_type)
...
game = StockMarket(machine_type='pdb')
game.reset()
game.run_loop()
