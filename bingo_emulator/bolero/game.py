...
from bingo_emulator.graphics.bolero import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.bolero.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh bolero")
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            if self.game.selector.position < 3:
                ...
                self.game.coin.step()
                if self.game.coin.position == 3:
                    self.game.selector.step()
                if self.game.coin.position == 9:
                    self.game.selector.step()
                if self.game.coin.position == 17:
                    self.game.selector.step()
            if self.game.switches.shutter.is_inactive():
    ...
    def sw_ballLift_active_for_500ms(self, sw):
        if self.game.switches.shooter.is_inactive():
            ...
            if self.game.ball_count.position == 5 and self.game.selector.position == 4:
                self.game.coils.lifter.enable()
            if self.game.ball_count.position <= 6 and self.game.selector.position == 5:
                self.game.coils.lifter.enable()
            if self.game.ball_count.position <= 7 and self.game.selector.position == 6:
                self.game.coils.lifter.enable()
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [56,292]
            image = pygame.image.load('bolero/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [200,282]
            image = pygame.image.load('bolero/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [364,283]
            image = pygame.image.load('bolero/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
class Bolero(procgame.game.BasicGame):
    """ Bolero was the second game United produced using the standard Bally playfield layout """
    def __init__(self, machine_type):
        super(Bolero, self).__init__(machine_type)
        ...
        #Coin unit needed since the extra balls are awarded on a guaranteed
        #basis.  First EB awarded with 3 extra coins, Second on 9 extra coins
        #Third on 17 extra coins.  This is based on Dennis Dodel's Bolero,
        #and it should be noted that the number of coins can be adjusted.
        self.coin = units.Stepper("coin", 20)

        self.replays = 0
        self.returned = False

    def reset(self):
        super(Bolero, self).reset()
