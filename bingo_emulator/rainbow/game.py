...
from bingo_emulator.graphics.rainbow import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh rainbow")
        else:
            if self.game.ball_count.position >= 4:
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [182,299]
            image = pygame.image.load('rainbow/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [246,298]
            image = pygame.image.load('rainbow/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [294,298]
            image = pygame.image.load('rainbow/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [381,298]
            image = pygame.image.load('rainbow/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
        if title5 == 1:
            pos = [449,298]
            image = pygame.image.load('rainbow/assets/title5_on.png').convert_alpha()
            screen.blit(image, pos)
...
class Rainbow(procgame.game.BasicGame):
    """ Bright Lights was the first bingo produced by Bally """
    def __init__(self, machine_type):
        super(Rainbow, self).__init__(machine_type)
