...
def display(s, replays=0, menu=False):
    ...
    if menu == True:
        backglass = pygame.image.load('golden_gate/assets/golden_gate_menu.png').convert()
        backglass.set_colorkey((255,0,252))
    else:
        if (s.game.anti_cheat.status == True):
            backglass = pygame.image.load('golden_gate/assets/golden_gate_gi.png').convert()
            backglass.set_colorkey((255,0,252))
        else:
            backglass = pygame.image.load('golden_gate/assets/golden_gate_off.png').convert()
            backglass.set_colorkey((255,0,252))
