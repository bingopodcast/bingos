...
from bingo_emulator.graphics.havana import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            graphics.havana.display(self)
            self.game.eb_play.disengage()
        else:
            self.game.tilt.disengage()
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.havana.display, param=self)
    ...
    def sw_trough8_closed(self, sw):
        if self.game.start.status == False:
            ...
            if self.game.letter_va.status == False:
                self.game.letter_va.engage(self.game)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh havana")
        else:
            if self.game.ball_return.position == 5 and self.game.card1_replay_counter.position == 0:
    ...
    def sw_redstar_active(self, sw):
        if self.game.red_star.status == True:
            ...
            if self.game.ball_count.position >= 5:
                self.search()
        if self.game.letter_na.status == False:
            self.game.letter_na.engage(self.game)
            self.game.sound.play('tilt')
            self.search()
        self.delay(name="display", delay=0.1, handler=graphics.havana.display, param=self)
    ...
    def regular_play(self, ball_return=False):
        ...
        if self.game.start.status == True:
            ...
            if self.game.eb_play.status == True:
                ...
                self.game.letter_va.disengage()
                self.game.letter_na.disengage()
            ...
            if self.game.before_fourth.status == False and self.game.before_fifth.status == False:
                ...
                if self.game.spotted.position == 10:
                    if self.game.left_special_card.status == False:
                        ...
                        self.game.sound.play('tilt')
                    if self.game.right_special_card.status == False:
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.havana.reel1, graphics.havana.reel10, graphics.havana.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.havana.reel1, graphics.havana.reel10, graphics.havana.reel100)
                    ...
                    if number == key:
                        ...
                        if self.game.selector.position >= self.card:
                            if s >= 2:
                                ...
                                break
        if self.game.letter_ha.status == True and self.game.letter_va.status == True:
            self.find_winner(0, 1, 0, 0, 0, 0, 2)
        if self.game.letter_ha.status == True and self.game.letter_va.status == True and self.game.letter_na.status == True:
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.havana.display, param=self)
        else:
    ...
    def check_mixer4(self):
        ...
        if m4 in [3,15,20,25,29,36,45]:
            if self.game.select_feature_relay.status == False: 
        ...
        if self.game.reflex2.position == 50:
            if m4 in [11,19,33]:
                if self.game.selection_feature_relay.status == False:
                    ...
                    self.game.sound.play('tilt')
        self.delay(name="display", delay=0, handler=graphics.havana.display, param=self)
        if m4 in [2,26,37,41,35,24,34,40,46,49]:
            if self.game.left_special_card.status == False:
                self.game.left_special_card.engage(self.game)
                self.game.sound.play('tilt')
            else:
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [1,3,12,14,17,21,24]:
                ...
                graphics.havana.animate_mixer1(self)
            if rivet in [2,6,10,11,20,23]:
                self.game.mixer2.step()
                graphics.havana.animate_mixer2(self)
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
                graphics.havana.animate_mixer3(self)
            graphics.havana.both_animation([self, start + num])
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
            graphics.havana.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Havana(procgame.game.BasicGame):
    """ Havana is a 25 hole Bally-style game. """
    def __init__(self, machine_type):
        super(Havana, self).__init__(machine_type)
        ...
        self.reflex1 = units.Stepper("reflex1", 50, "Havana", "continuous")
        self.reflex2 = units.Stepper("reflex2", 50, "Havana")
        ...
        self.spotted = units.Stepper("spotted", 10, "havana", "continuous")

        self.super_card = units.Stepper("super_card", 8)
        self.ball_return = units.Stepper("ball_return", 5)
        ...
        self.letter_va = units.Relay("letter_va")
        self.letter_na = units.Relay("letter_na")
