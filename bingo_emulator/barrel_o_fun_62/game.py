...
from bingo_emulator.graphics.barrel_o_fun_62 import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.barrel_o_fun_62.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh barrel_o_fun_62")
...
#        self.cancel_delayed(name="blink_title")
        graphics.barrel_o_fun_62.display(self)
        self.tilt_actions()
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [418,264]
            image = pygame.image.load('barrel_o_fun_62/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [543,250]
            image = pygame.image.load('barrel_o_fun_62/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [656,250]
            image = pygame.image.load('barrel_o_fun_62/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
class BarrelOFun62(procgame.game.BasicGame):
    """ Barrel O' Fun '61 was an Ohio Dime Game with a replay button  """
    def __init__(self, machine_type):
        super(BarrelOFun62, self).__init__(machine_type)
...
game = BarrelOFun62(machine_type='pdb')
game.reset()
game.run_loop()
