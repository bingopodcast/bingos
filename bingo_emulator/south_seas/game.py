...
from bingo_emulator.graphics.south_seas import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            self.scan_all()
        self.delay(name="display", delay=0.1, handler=graphics.south_seas.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh south_seas")
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            ...
            self.game.diagonals_relay.disengage()
            self.game.diagonal_scoring.reset()
...
#        self.cancel_delayed(name="blink_title")
        graphics.south_seas.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.south_seas.reel1, graphics.south_seas.reel10, graphics.south_seas.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.south_seas.reel1, graphics.south_seas.reel10, graphics.south_seas.reel100)
                ...
                for number in self.holes:
                    if number == key:
                        ...
                        if self.game.selector.position >= self.card:
                            if s >= 2:
                                self.find_winner(s, self.card, self.corners, self.diagonals)
                                break
        if self.game.name.position == 6:
            self.find_winner(0, 1, 0, 0, 1)
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)

    def find_winner(self, relays, card, corners, diagonals, name=0):
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if relays == 3:
                if not corners:
                    if diagonals:
                        if self.game.diagonals.status == True:
                            if self.game.diagonal_scoring.position >= 6:
    ...
    def diagonal_replay_step_up(self, number):
        if number >= 1:
            ...
            if self.game.replays == 899:
                number = 0
            self.delay(name="diagonal_replay_step_up", delay=0.1, handler=self.diagonal_replay_step_up, param=number)
        else:
            self.game.search_index.disengage()
    ...
    def closed_search_relays(self, rivets):
        ...
        self.pos = {}
        self.pos[0] = {}
        self.pos[1] = {2:1, 15:2, 19:3}
        self.pos[2] = {1:1, 18:2, 14:3, 24:4}
        self.pos[3] = {7:1, 13:2, 12:3, 8:4}
        self.pos[4] = {5:1, 21:2, 25:3}
        self.pos[5] = {15:1, 20:2, 25:3}
        self.pos[6] = {4:1, 14:2, 12:3, 17:4}
        self.pos[7] = {10:1, 18:2, 13:3, 23:4}
        self.pos[8] = {2:1, 22:2, 5:3}
        self.pos[9] = {}
        self.pos[10] = {}
        self.pos[11] = {}
        self.pos[12] = {}
        ...
        self.pos[29] = {6:1, 3:2, 11:3, 9:4}
        self.pos[30] = {3:1, 11:2, 9:3, 6:4}
        self.pos[31] = {11:1, 9:2, 6:3, 3:4}
        self.pos[32] = {}
        self.pos[33] = {}
        self.pos[34] = {}
        self.pos[35] = {}
        ...
        if rivets in range(1,9):
            diagonals = True
        if rivets in range(28,32):
            corners = True

        return (self.pos[rivets], card, corners, diagonals)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [284,249]
            image = pygame.image.load('south_seas/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [229,275]
            image = pygame.image.load('south_seas/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [347,272]
            image = pygame.image.load('south_seas/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.south_seas.display, param=self)
        else:
    ...
    def features_probability(self):
        ...
        if sd == 30:
            self.game.diagonal_scoring.step()
        if sd == 0 or sd == 25:
        ...
        if self.game.diagonal_scoring.position < 5:
            if self.game.diagonals_relay.status == False:
                self.game.diagonals_relay.engage(self.game)
            else:
                self.game.diagonal_scoring.step()
        else:
            self.game.diagonal_scoring.step()
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.south_seas.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.south_seas.display, param=self)
            s -= 1
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.south_seas.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.south_seas.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.south_seas.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.south_seas.display, param=self)
            s -= 1
...
class SouthSeas(procgame.game.BasicGame):
    def __init__(self, machine_type):
        super(SouthSeas, self).__init__(machine_type)
        ...
        #Odds stepper
        self.odds = units.Stepper("odds", 8, 'south_seas')
        ...
        self.spotted = units.Stepper("spotted", 10, "south_seas", "continuous")
        ...
        self.diagonals_relay = units.Relay("diagonals")
        self.diagonal_scoring = units.Stepper("diagonals_scoring", 6)
...
game = SouthSeas(machine_type='pdb')
game.reset()
game.run_loop()
