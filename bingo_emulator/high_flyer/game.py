...
from bingo_emulator.graphics.high_flyer import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.high_flyer.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh high_flyer")
        else:
            if self.game.ball_count.position >= 4:
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            if self.game.selector.position < 8:
...
##        self.cancel_delayed(name="blink_title")
        graphics.high_flyer.display(self, 0)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.high_flyer.reel1, graphics.high_flyer.reel10, graphics.high_flyer.reel100, graphics.high_flyer.reel1000)
                self.game.coils.registerDown.pulse()
                number -= 1
                graphics.high_flyer.display(self)
    ...
    def replay_step_up(self):
        if self.game.replays < 8999:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.high_flyer.reel1, graphics.high_flyer.reel10, graphics.high_flyer.reel100, graphics.high_flyer.reel1000)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
        ...
        if self.game.switches.left.is_inactive() and self.game.switches.right.is_inactive():
            if i[0] == 1 and self.game.card1_replay_counter.position > 0:
                ...
                if i[0] == 1 and self.game.card1_missed.status == True:
                    ...
                    graphics.high_flyer.blink_double(self)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [61,251]
            image = pygame.image.load('high_flyer/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [218,251]
            image = pygame.image.load('high_flyer/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [406,252]
            image = pygame.image.load('high_flyer/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [529,266]
            image = pygame.image.load('high_flyer/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class HighFlyer(procgame.game.BasicGame):
    """ Bright Lights was the first bingo produced by Bally """
    def __init__(self, machine_type):
        super(HighFlyer, self).__init__(machine_type)
        ...
        self.selector = units.Stepper("selector", 8)
...
game = HighFlyer(machine_type='pdb')
game.reset()
game.run_loop()
