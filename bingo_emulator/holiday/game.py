...
from bingo_emulator.graphics.holiday import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        if self.game.special_feature.status == False:
            self.regular_play()
        else:
            self.sw_green_active(sw)
        self.delay(name="display", delay=0.1, handler=graphics.holiday.display, param=self)
    ...
    def sw_green_active(self, sw):
        if self.game.special_1.status == False and self.game.special_2.status == False and self.game.special_3.status == False and self.game.special_4.status == False and self.game.special_5.status == False and self.game.special_6.status == False:
            if self.game.special_feature.status == True:
                if self.game.replays > 0 or self.game.switches.freeplay.is_active() or self.game.switches.coin.is_active():
                    self.replay_step_down()
                    sf = random.randint(1,6)
                    if sf == 1:
                        self.game.special_1.engage(self.game)
                    elif sf == 2:
                        self.game.special_2.engage(self.game)
                    elif sf == 3:
                        self.game.special_3.engage(self.game)
                    elif sf == 4:
                        self.game.special_4.engage(self.game)
                    elif sf == 5:
                        self.game.special_5.engage(self.game)
                    elif sf == 6:
                        self.game.special_6.engage(self.game)

                    self.delay(name="display", delay=0.1, handler=graphics.holiday.display, param=self)
                    self.game.special_feature.disengage()
                    self.delay(name="display", delay=0.1, handler=graphics.holiday.display, param=self)
            else:
                print "HERE"
                self.game.special_feature.engage(self.game)
                self.delay(name="display", delay=0.1, handler=graphics.holiday.display, param=self)
                self.sw_green_active(sw)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh holiday")
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            ...
            self.game.special_feature.disengage()
            self.game.special_1.disengage()
            self.game.special_2.disengage()
            self.game.special_3.disengage()
            self.game.special_4.disengage()
            self.game.special_5.disengage()
            self.game.special_6.disengage()
...
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.holiday.reel1, graphics.holiday.reel10, graphics.holiday.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.holiday.reel1, graphics.holiday.reel10, graphics.holiday.reel100)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def find_winner(self, relays, card):
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if card == 1:
                if self.game.special_1.status == True:
                    hit = hit * 3
                if relays == 3:
                ...
                if relays == 5:
                    if self.game.card1_replay_counter.position < hit:
                        ...
                        self.card1_replay_step_up(hit - self.game.card1_replay_counter.position)
            if card == 2:
                if self.game.special_2.status == True:
                    hit = hit * 3
                if relays == 3:
                ...
                if relays == 5:
                    if self.game.card2_replay_counter.position < hit:
                        ...
                        self.card2_replay_step_up(hit - self.game.card2_replay_counter.position)
            if card == 3:
                if self.game.special_3.status == True:
                    hit = hit * 3
                if relays == 3:
                ...
                if relays == 5:
                    if self.game.card3_replay_counter.position < hit:
                        ...
                        self.card3_replay_step_up(hit - self.game.card3_replay_counter.position)
            if card == 4:
                if self.game.special_4.status == True:
                    hit = hit * 3
                if relays == 3:
                ...
                if relays == 5:
                    if self.game.card4_replay_counter.position < hit:
                        ...
                        self.card4_replay_step_up(hit - self.game.card4_replay_counter.position)
            if card == 5:
                if self.game.special_5.status == True:
                    hit = hit * 3
                if relays == 3:
                ...
                if relays == 5:
                    if self.game.card5_replay_counter.position < hit:
                        ...
                        self.card5_replay_step_up(hit - self.game.card5_replay_counter.position)
            if card == 6:
                if self.game.special_6.status == True:
                    hit = hit * 3
                if relays == 3:
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [182,299]
            image = pygame.image.load('holiday/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [246,298]
            image = pygame.image.load('holiday/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [294,298]
            image = pygame.image.load('holiday/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [381,298]
            image = pygame.image.load('holiday/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
        if title5 == 1:
            pos = [449,298]
            image = pygame.image.load('holiday/assets/title5_on.png').convert_alpha()
            screen.blit(image, pos)
...
class Holiday(procgame.game.BasicGame):
    """ Bright Lights was the first bingo produced by Bally """
    def __init__(self, machine_type):
        super(Holiday, self).__init__(machine_type)
        ...
        self.special_feature = units.Relay("special_feature")
        self.special_1 = units.Relay("special_1")
        self.special_2 = units.Relay("special_2")
        self.special_3 = units.Relay("special_3")
        self.special_4 = units.Relay("special_4")
        self.special_5 = units.Relay("special_5")
        self.special_6 = units.Relay("special_6")

        self.replays = 0
        self.returned = False

    def reset(self):
        super(Holiday, self).reset()
