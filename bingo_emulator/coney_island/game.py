...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.coney_island.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh coney_island")
...
#        self.cancel_delayed(name="blink_title")
        graphics.coney_island.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.coney_island.reel1, graphics.coney_island.reel10, graphics.coney_island.reel100)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [222,332]
            image = pygame.image.load('coney_island/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [394,334]
            image = pygame.image.load('coney_island/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [214,388]
            image = pygame.image.load('coney_island/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [342,390]
            image = pygame.image.load('coney_island/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.coney_island.eb_animation, param=self.game.extra_ball.position)
            self.delay(name="display", delay=0.1, handler=graphics.coney_island.display, param=self)
            s -= 1
    ...
    def eb_probability(self):
        if self.game.reflex.connected_rivet() == 4:
            return 1
        elif self.game.reflex.connected_rivet() == 3 and self.game.mixer.connected_rivet("coney_island") == 3:
            return 1
        elif self.game.reflex.connected_rivet() == 2 and self.game.mixer.connected_rivet("coney_island") == 2:
            return 1
        elif self.game.reflex.connected_rivet() == 1 and self.game.mixer.connected_rivet("coney_island") == 1:
            return 1
        else:
            return 0
