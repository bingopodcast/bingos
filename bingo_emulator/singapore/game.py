...
from bingo_emulator.graphics.singapore import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            self.scan_all()
        self.delay(name="display", delay=0.1, handler=graphics.singapore.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh singapore")
        else:
...
#        self.cancel_delayed(name="blink_title")
        graphics.singapore.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.singapore.reel1, graphics.singapore.reel10, graphics.singapore.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.singapore.reel1, graphics.singapore.reel10, graphics.singapore.reel100)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
        ...
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if relays == 2:
                if supercard == 1:
                    ...
                    if self.game.super_card.position >= 6:
                        if self.game.card1_replay_counter.position < threeodds:
                            self.game.search_index.engage(self.game)
                            self.card1_replay_step_up(threeodds - self.game.card1_replay_counter.position)
                elif supercard == 4:
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [284,249]
            image = pygame.image.load('singapore/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [229,275]
            image = pygame.image.load('singapore/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [347,272]
            image = pygame.image.load('singapore/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.singapore.display, param=self)
        else:
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.singapore.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.singapore.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.singapore.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.singapore.display, param=self)
            s -= 1
...
class Singapore(procgame.game.BasicGame):
    """ Singapore is the second United game with a standard Bally 25 hole playfield.  It's not _exactly_ the same, but close enough. """
    def __init__(self, machine_type):
        super(Singapore, self).__init__(machine_type)
        ...
        self.reflex1 = units.Stepper("reflex1", 50, "Singapore", "continuous")
        self.reflex2 = units.Stepper("reflex2", 50, "Singapore")
        ...
        self.spotted = units.Stepper("spotted", 10, "singapore", "continuous")
...
game = Singapore(machine_type='pdb')
game.reset()
game.run_loop()
