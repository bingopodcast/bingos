...
class MulticardBingo(procgame.game.Mode):
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [0,4,7,8,16,18]:
                ...
                graphics.singapore.animate_mixer4(self)
            if rivet in [1,3,12,14,17,21,24]:
                self.game.mixer1.step()
                graphics.singapore.animate_mixer1(self)
            if rivet in [2,6,10,11,20,23]:
                self.game.mixer2.step()
                graphics.singapore.animate_mixer2(self)
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
                graphics.singapore.animate_mixer3(self)
            graphics.singapore.both_animation([self, start + num])
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
            graphics.singapore.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Singapore(procgame.game.BasicGame):
    """ Singapore is a United game with Special Cards """
