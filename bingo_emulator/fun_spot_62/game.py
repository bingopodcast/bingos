...
from bingo_emulator.graphics.fun_spot_62 import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def play_replays(self):
        if self.game.replays > 0 and self.game.selector.position < 6:
            ...
            self.delay(name="display", delay=0.2, handler=graphics.fun_spot_62.display, param=self)
    ...
    def sw_startButton_active(self, sw):
        if self.game.replays > 0 or self.game.switches.freeplay.is_active():
            self.regular_play()
            self.delay(name="display", delay=0.1, handler=graphics.fun_spot_62.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh fun_spot_62")
    ...
    def check_spotting(self):
        ...
        if self.game.spotting.position == 3:
            ...
            if 21 not in self.holes:
                self.holes.append(21)
        elif self.game.spotting.position == 17:
            if 17 not in self.holes:
                self.holes.append(17)
        elif self.game.spotting.position == 21:
            if 21 not in self.holes:
                self.holes.append(21)
        elif self.game.spotting.position == 22:
            if 15 not in self.holes:
                self.holes.append(15)
        elif self.game.spotting.position == 24:
            if 16 not in self.holes:
                self.holes.append(16)
        elif self.game.spotting.position == 25:
            if 22 not in self.holes:
                self.holes.append(22)
        elif self.game.spotting.position == 29:
            if 20 not in self.holes:
                self.holes.append(20)
        elif self.game.spotting.position == 31:
            if 20 not in self.holes:
                self.holes.append(20)
            ...
            if 22 not in self.holes:
                self.holes.append(22)
        elif self.game.spotting.position == 38:
            if 17 not in self.holes:
                self.holes.append(17)
        elif self.game.spotting.position == 41:
            if 20 not in self.holes:
                self.holes.append(20)
        elif self.game.spotting.position == 43:
            if 21 not in self.holes:
                self.holes.append(21)
...
#        self.cancel_delayed(name="blink_title")
        graphics.fun_spot_62.display(self)
        self.tilt_actions()
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [418,264]
            image = pygame.image.load('fun_spot_62/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [543,250]
            image = pygame.image.load('fun_spot_62/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [656,250]
            image = pygame.image.load('fun_spot_62/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
class FunSpot62(procgame.game.BasicGame):
    """ Barrel O' Fun '61 was an Ohio Dime Game with a replay button  """
    def __init__(self, machine_type):
        super(FunSpot62, self).__init__(machine_type)
...
game = FunSpot62(machine_type='pdb')
game.reset()
game.run_loop()
