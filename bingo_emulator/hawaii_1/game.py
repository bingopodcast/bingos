...
from bingo_emulator.graphics.hawaii_1 import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            graphics.hawaii_1.display(self)
            self.game.eb_play.disengage()
        else:
            self.game.tilt.disengage()
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.hawaii_1.display, param=self)
    ...
    def sw_trough8_closed(self, sw):
        if self.game.start.status == False:
            ...
            if self.game.letter_wa.status == False:
                self.game.letter_wa.engage(self.game)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh hawaii_1")
        else:
    ...
    def sw_redstar_active(self, sw):
        if self.game.red_star.status == True:
            ...
            if self.game.ball_count.position >= 5:
                self.search()
        if self.game.letter_ii.status == False:
            self.game.letter_ii.engage(self.game)
            self.game.sound.play('tilt')
            self.search()
        self.delay(name="display", delay=0.1, handler=graphics.hawaii_1.display, param=self)
    ...
    def sw_yellowstar_active(self, sw):
        ...
        if self.game.letter_ha.status == False:
            self.game.letter_ha.engage(self.game)
            self.game.sound.play('tilt')
            self.search()
        self.delay(name="display", delay=0.1, handler=graphics.hawaii_1.display, param=self)
    ...
    def regular_play(self, ball_return=False):
        ...
        if self.game.start.status == True:
            ...
            if self.game.eb_play.status == True:
                ...
                self.game.letter_ha.disengage()
                self.game.letter_wa.disengage()
                self.game.letter_ii.disengage()
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.hawaii_1.reel1, graphics.hawaii_1.reel10, graphics.hawaii_1.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.hawaii_1.reel1, graphics.hawaii_1.reel10, graphics.hawaii_1.reel100)
                    ...
                    if number == key:
                        ...
                        if self.game.selector.position >= self.card:
                            if s >= 3:
                                ...
                                break
        if self.game.letter_ha.status == True and self.game.letter_wa.status == True:
            self.find_winner(0, 1, 0, 0, 0, 2)
        if self.game.letter_ha.status == True and self.game.letter_wa.status == True and self.game.letter_ii.status == True:
            self.find_winner(0, 1, 0, 0, 0, 3)
            ...
            if r2 == 1:
                ...
                if m2 == 1:
                    ...
                    if self.game.diamond_diagonal.status == False:
                        ...
                        self.game.sound.play('tilt')
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.hawaii_1.display, param=self)
        else:
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [0,4,7,8,16,18]:
                ...
                graphics.hawaii_1.animate_mixer4(self)
            if rivet in [1,3,12,14,17,21,24]:
                self.game.mixer1.step()
                graphics.hawaii_1.animate_mixer1(self)
            if rivet in [2,6,10,11,20,23]:
                self.game.mixer2.step()
                graphics.hawaii_1.animate_mixer2(self)
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
                graphics.hawaii_1.animate_mixer3(self)
            graphics.hawaii_1.both_animation([self, start + num])
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
            graphics.hawaii_1.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Hawaii1(procgame.game.BasicGame):
    """ Hawaii is a United game with a standard 25 hole Bally-style playfield. """
    def __init__(self, machine_type):
        super(Hawaii1, self).__init__(machine_type)
        ...
        self.reflex1 = units.Stepper("reflex1", 50, "Hawaii1", "continuous")
        self.reflex2 = units.Stepper("reflex2", 50, "Hawaii1")
        ...
        self.spotted = units.Stepper("spotted", 10, "hawaii_1", "continuous")
        ...
        self.letter_ha = units.Relay("letter_ha")
        self.letter_wa = units.Relay("letter_wa")
        self.letter_ii = units.Relay("letter_ii")
