...
class SinglecardBingo(procgame.game.Mode):
    ...
    def features_spotting(self):
        ...
        if self.game.magic_screen_feature.position in [4,6,8,10]:
            #SUPER SECTIONS!
        ...
        #1-7 Feature
        if sd in [1,5,20,28,39,42,43,49]:
            if self.game.one_seven_feature.status == False:
    ...
    def step_magic_screen(self, number):
        if number >= 1:
            self.game.magic_screen_feature.step()
            number -= 1
            if self.game.magic_screen_feature.position >= 1 and self.game.magic_screen_feature <= 4:
                number = 4 - self.game.magic_screen_feature.position
            self.delay(name="display", delay=0.1, handler=graphics.ballerina.display, param=self)
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.ballerina.both_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_screen_right(self, args):
        ...
        num = args[1]
        if num >= 0:
            graphics.ballerina.screen_animation([self, num, "right"])
            self.cancel_delayed(name="display")
            num = num - 1
    ...
    def animate_screen_left(self, args):
        ...
        num = args[1]
        if num >= 0:
            graphics.ballerina.screen_animation([self, num, "left"])
            self.cancel_delayed(name="display")
            num = num - 1
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.ballerina.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Ballerina(procgame.game.BasicGame):
    """ Ballerina is the only game with the 1-7 7-1 feature """
    def __init__(self, machine_type):
        ...
        self.cu = 1
        self.cam4 = units.Stepper("cam4", 3, "ballerina", "continuous")
