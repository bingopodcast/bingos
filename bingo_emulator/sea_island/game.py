...
from bingo_emulator.graphics.sea_island import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.delay(name="display", delay=0.1, handler=graphics.sea_island.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh sea_island")
        else:
    ...
    def magic_screen_reset_up(self, number):
        if number != 2:
            self.game.magic_screen.step()
            self.delay(name="display", delay=0, handler=graphics.sea_island.display, param=self)
            number += 1
...
#        self.cancel_delayed(name="blink_title")
        graphics.sea_island.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.sea_island.reel1, graphics.sea_island.reel10, graphics.sea_island.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.sea_island.reel1, graphics.sea_island.reel10, graphics.sea_island.reel100)
...
#        self.cancel_delayed(name="blink_title")
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.sea_island.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.sea_island.display, param=self)
            s -= 1
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.sea_island.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.sea_island.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.sea_island.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.sea_island.display, param=self)
            s -= 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [167,257]
            image = pygame.image.load('sea_island/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [241,290]
            image = pygame.image.load('sea_island/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [346,298]
            image = pygame.image.load('sea_island/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [431,264]
            image = pygame.image.load('sea_island/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class SeaIsland(procgame.game.BasicGame):
    """ Palm Beach was the first game with Super Cards """
    def __init__(self, machine_type):
        super(SeaIsland, self).__init__(machine_type)
        ...
        #Odds steppers
        self.red_odds = units.Stepper("red_odds", 8, 'sea_island')
        self.yellow_odds = units.Stepper("yellow_odds", 8, 'sea_island')
        self.green_odds = units.Stepper("green_odds", 8, 'sea_island')
        ...
        self.magic_screen_feature = units.Stepper("magic_screen_feature", 11)
...
game = SeaIsland(machine_type='pdb')
game.reset()
game.run_loop()
