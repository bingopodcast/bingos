...
from bingo_emulator.graphics.beach_time import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.delay(name="display", delay=0.1, handler=graphics.beach_time.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh beach_time")
    ...
    def regular_play(self):
        ...
        if self.game.score_feature.position == 4 and self.game.ball_count.position <= 2:
            pass
        elif self.game.score_feature.position == 5 and self.game.ball_count.position <= 3:
            pass
        elif self.game.score_feature.position == 6 and self.game.ball_count.position <= 4:
            pass
        else:
            ...
            if self.game.start.status == True:
                ...
                graphics.beach_time.display(self)
                self.check_lifter_status()
            else:
                if 16 in self.holes and self.game.ballyhole.status == True:
                    ...
                    self.step_ms(9)
                    self.holes = []
                    self.game.magic_line_f.engage(self.game)
                    ...
                    if self.game.line_f.position not in [0,2]:
                        self.game.sound.play('square')
                        self.game.line_f.step()
                    ...
                    self.holes = []
                    self.game.magic_line_f.disengage()
                    self.game.red_replay_counter.reset()
                    self.game.yellow_replay_counter.reset()
                    self.game.green_replay_counter.reset()
                    self.game.corners.disengage()
    ...
    def magic_squarea_reset(self, number):
        if number != 0:
            self.game.sound.play('square')
            self.game.square_a.step()
            self.delay(name="display", delay=0, handler=graphics.beach_time.display, param=self)
            self.cancel_delayed("squarea_animation")
            self.animate_squarea([self.game,1,1])
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.beach_time.reel1, graphics.beach_time.reel10, graphics.beach_time.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.beach_time.reel1, graphics.beach_time.reel10, graphics.beach_time.reel100)
            ...
            if self.game.switches.drawer.is_inactive():
                if self.game.ball_count.position >= 1:
                    ...
                    if self.game.selection_feature.position == 9:
                        max_ball = 6
                    if self.game.ball_count.position < max_ball:
                        if self.game.magic_line_f.status == True:
                            self.game.sound.play('square')
                            self.game.line_f.step()
                            self.cancel_delayed("line_animation")
                            self.animate_line([self.game,1,1])
    ...
    def closed_search_relays(self, rivets, c):
        ...
        if self.game.line_f.position == 0 or self.game.line_f.position == 2:
            self.p5 = 15
            self.q5 = 18
            self.r5 = 17
            self.s5 = 20
            self.t5 = 10
        elif self.game.line_f.position == 1:
            self.p5 = 10
            self.q5 = 15
            self.r5 = 18
            self.s5 = 17
        ...
        self.pos[3] = {self.r1:1, self.r2:2, self.r3:3, self.r4:4, self.r5:5}
    ...
    def red_odds_probability(self):
        ...
        if self.game.red_odds.position == 1:
            ...
            if self.game.red_odds.position < 5:
                ...
                if sd in [4,8,10,14,15,19,22,23,24,27,29,31,32] or sd in range(34,38) or sd in [41,42,46,47,48]:
                    return 1
                else:
                    return 0
            elif self.game.red_odds.position == 6:
    ...
    def check_mixer2(self):
        ...
        if self.game.selection_feature.position == 9:
            if m2 in range(3,6) or m2 in [10,13,15,17,19,21,23,24,1]:
                return 1
        if ms <= 5:
    ...
    def features_spotting(self):
        sd = self.game.spotting.position
        ms = self.game.magic_squares_feature.position
        ss = self.game.score_feature.position
        if ms <= 2:
            self.step_ms(1)
        if ms == 3:
        ...
        if self.game.selection_feature.position == 8:
            if sd == 1:
                self.step_sf(1)
        if sd in [10,16,18,26,28,48,49]:
            if self.game.magic_squares_feature.position >= 5:
                if self.game.magic_line_f.status == False:
                    self.game.magic_line_f.engage(self.game)
                    self.game.sound.play('tilt')
        if sd in [4,28]:
        ...
        if ss < 2:
            self.step_ss(1)
        if ss == 2:
            if sd in [5,7,11,12,24,31,32,34,36,37,38,42,43,44]:
                self.step_ss(1)
        if ss == 3:
            # There's a typo on the schematic - there's no wire 15-11 on the spotting disc.  *sigh* I used 15-8 instead on the outer edge of the SD.
            if sd in [4,6,9,20,25,30,39]:
                self.step_ss(1)
        if ss == 4:
            if sd in [6,9,20,25,30]:
                self.step_ss(1)
        if ss == 5:
            if sd in [13,21]:
                self.step_ss(1)
    ...
    def step_ss(self, number):
        if number >= 1:
            self.game.score_feature.step()
            number -= 1
            self.delay(name="display", delay=0.1, handler=graphics.beach_time.display, param=self)
            self.delay(name="step_ss", delay=0.1, handler=self.step_ss, param=number)
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.beach_time.both_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_line(self, args):
        ...
        line = args[2]
        if num < 55:
            graphics.beach_time.line_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
    ...
    def animate_squarea(self, args):
        ...
        square = args[2]
        if num < 55:
            graphics.beach_time.squarea_animation([self, num * -1, square])
            self.cancel_delayed(name="display")
            num = num + 1
    ...
    def animate_squareb(self, args):
        ...
        square = args[2]
        if num < 55:
            graphics.beach_time.squareb_animation([self, num * -1, square])
            self.cancel_delayed(name="display")
            num = num + 1
            args = [self.game,num,square]
    ...
    def animate_squarec(self, args):
        ...
        square = args[2]
        if num < 62:
            graphics.beach_time.squarec_animation([self, num * -1, square])
            self.cancel_delayed(name="display")
            num = num + 1
            args = [self.game,num,square]
    ...
    def animate_squared(self, args):
        ...
        square = args[2]
        if num < 60:
            graphics.beach_time.squared_animation([self, num * -1, square])
            self.cancel_delayed(name="display")
            num = num + 1
            args = [self.game,num,square]
    ...
    def animate_squaree(self, args):
        ...
        square = args[2]
        if num < 60:
            graphics.beach_time.squaree_animation([self, num * -1, square])
            self.cancel_delayed(name="display")
            num = num + 1
            args = [self.game,num,square]
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.beach_time.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def check_eb_spotting(self):
        sd = self.game.spotting.position
        if sd == 50:
            if self.game.mixer3.position in [0,23]:
                self.step_eb(9 - self.game.extra_ball.position)
        elif sd == 18:
            if self.game.mixer3.position == [5,11,15,22]:
...
class BeachTime(procgame.game.BasicGame):
    """ Beach Time was the most complex Magic Squares game """
    def __init__(self, machine_type):
        super(BeachTime, self).__init__(machine_type)
        ...
        #Odds stepper
        self.red_odds = units.Stepper("red_odds", 8, 'beach_time')
        self.yellow_odds = units.Stepper("yellow_odds", 8, 'beach_time')
        self.green_odds = units.Stepper("green_odds", 8, 'beach_time')
        ...
        self.score_feature = units.Stepper("score_feature", 6)

        self.selector = units.Stepper("selector", 1)

        self.square_a = units.Stepper("square_a", 3, "beach_time", "continuous")
        self.square_b = units.Stepper("square_b", 3, "beach_time", "continuous")
        self.square_c = units.Stepper("square_c", 3, "beach_time", "continuous")
        self.square_d = units.Stepper("square_d", 3, "beach_time", "continuous")
        self.square_e = units.Stepper("square_e", 3, "beach_time", "continuous")
        self.line_f = units.Stepper("line_f", 3, "beach_time", "continuous")

        self.ballyhole = units.Relay("ballyhole")

        self.magic_squares_feature = units.Stepper("magic_squares_feature", 9)
        self.magic_line_f = units.Relay("magic_line_f")
...
game = BeachTime(machine_type='pdb')
game.reset()
game.run_loop()
