...
from bingo_emulator.graphics.wall_street import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.wall_street.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh wall_street")
        else:
            if self.game.ball_count.position >= 4:
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            if self.game.selector.position < 6:
                ...
                if self.check_spotted() == True:
                    if self.game.selector.position == 1:
                        ...
                        self.game.twenty.engage(self.game)
                        self.holes.append(20)
                    elif self.game.selector.position == 4:
                        self.game.seventeen.engage(self.game)
                        self.holes.append(17)
                    elif self.game.selector.position == 5:
                        self.game.twentyone.engage(self.game)
            ...
            self.game.twenty.disengage()
            self.game.seventeen.disengage()
            self.game.twentyone.disengage()
...
##        self.cancel_delayed(name="blink_title")
        graphics.wall_street.display(self, 0)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.wall_street.reel1, graphics.wall_street.reel10, graphics.wall_street.reel100, graphics.wall_street.reel1000)
                self.game.coils.registerDown.pulse()
                number -= 1
                graphics.wall_street.display(self)
    ...
    def replay_step_up(self):
        if self.game.replays < 8999:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.wall_street.reel1, graphics.wall_street.reel10, graphics.wall_street.reel100, graphics.wall_street.reel1000)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
        ...
        if self.game.switches.left.is_inactive() and self.game.switches.right.is_inactive():
            if i[0] == 1 and self.game.card1_replay_counter.position > 0:
                ...
                if i[0] == 1 and self.game.card1_missed.status == True:
                    ...
                    graphics.wall_street.blink_double(self)
    ...
    def check_double_probability(self):
        ...
        if pos == 1 or pos == 4 or pos == 5 or pos == 8 or pos == 11 or pos == 15 or pos == 17 or pos == 19 or pos == 21 or pos == 24:
            return 1
    ...
    def check_spotted(self):
        pos = self.game.probability.position
        print pos
        if self.game.cu:
            if pos == 18 or pos == 19 or pos == 20 or pos == 22 or pos == 23 or pos == 24:
                return 1
        else:
            if pos == 22 or pos == 23 or pos == 24:
                return 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [61,251]
            image = pygame.image.load('wall_street/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [218,251]
            image = pygame.image.load('wall_street/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [406,252]
            image = pygame.image.load('wall_street/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [529,266]
            image = pygame.image.load('wall_street/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class WallStreet(procgame.game.BasicGame):
    """ Bright Lights was the first bingo produced by Bally """
    def __init__(self, machine_type):
        super(WallStreet, self).__init__(machine_type)
        ...
        self.twenty = units.Relay("twenty")
        self.seventeen = units.Relay("seventeen")
        self.twentyone = units.Relay("twentyone")
        self.twentytwo = units.Relay("twentytwo")

        self.returned = False

    def reset(self):
        super(WallStreet, self).reset()
...
game = WallStreet(machine_type='pdb')
game.reset()
game.run_loop()
