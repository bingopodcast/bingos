...
from bingo_emulator.graphics.u345 import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.u345.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh u345")
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [298,215]
            image = pygame.image.load('u345/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [181,239]
            image = pygame.image.load('u345/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [385,243]
            image = pygame.image.load('u345/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
class u345(procgame.game.BasicGame):
    """ 345 was an early experiment with the Bally playfield layout """
    def __init__(self, machine_type):
        super(u345, self).__init__(machine_type)
