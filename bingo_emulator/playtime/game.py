...
from bingo_emulator.graphics.playtime import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.delay(name="display", delay=0.1, handler=graphics.playtime.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh playtime")
    ...
    def regular_play(self):
        ...
        self.cancel_delayed(name="orange_replay_step_up")
        ...
        if self.game.start.status == True:
            ...
            self.holes = []
            self.game.orange_replay_counter.reset()
    ...
    def magic_squarea_reset(self, number):
        if number != 0:
            self.game.square_a.step()
            self.delay(name="display", delay=0, handler=graphics.playtime.display, param=self)
...
#        self.cancel_delayed(name="blink_title")
        graphics.playtime.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.playtime.reel1, graphics.playtime.reel10, graphics.playtime.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.playtime.reel1, graphics.playtime.reel10, graphics.playtime.reel100)
            ...
            for key in self.wipers:
                for number in self.holes:
                    if number == key:
                        ...
                        if self.game.selector.position >= 1:
                            if s >= 3:
                                self.find_winner(s, self.card, self.corners, self.yellow, self.orange, self.red)
                                break
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)

    def find_winner(self, relays, card, corners, yellow, orange, green):
        if self.game.search_index.status == False and self.game.replays < 899:
            if orange == True:
                if self.game.orange_odds.position == 1:
                    threeodds = 4
                    fourodds = 16
                    fiveodds = 75
                elif self.game.orange_odds.position == 2:
                    threeodds = 6
                    fourodds = 20
                    fiveodds = 75
                elif self.game.orange_odds.position == 3:
                    threeodds = 8
                    fourodds = 24
                    fiveodds = 96
                elif self.game.orange_odds.position == 4:
                    threeodds = 16
                    fourodds = 50
                    fiveodds = 96
                elif self.game.orange_odds.position == 5:
                    threeodds = 32
                    fourodds = 96
                    fiveodds = 200
                elif self.game.orange_odds.position == 6:
                    threeodds = 64
                    fourodds = 144
                    fiveodds = 300
                elif self.game.orange_odds.position == 7:
                    threeodds = 120
                    fourodds = 240
                    fiveodds = 450
                elif self.game.orange_odds.position == 8:
                    threeodds = 192
                    fourodds = 480
                    fiveodds = 600
            elif yellow == True or corners == True:
            ...
            if relays == 3:
                if not corners:
                    if orange:
                        if self.game.orange_replay_counter.position < threeodds:
                            self.game.search_index.engage(self.game)
                            self.orange_replay_step_up(threeodds - self.game.orange_replay_counter.position)
                    elif yellow:
            ...
            if relays == 4:
                if corners and self.game.corners.status == True:
                    ...
                    if not corners:
                        if orange:
                            if self.game.orange_replay_counter.position < fourodds:
                                self.game.search_index.engage(self.game)
                                self.orange_replay_step_up(fourodds - self.game.orange_replay_counter.position)
                        elif yellow:
            ...
            if relays == 5:
                if orange:
                    if self.game.orange_replay_counter.position < fiveodds:
                        self.game.search_index.engage(self.game)
                        self.orange_replay_step_up(fiveodds - self.game.orange_replay_counter.position)
                elif yellow:
                    if self.game.yellow_replay_counter.position < fiveodds:
                        self.game.search_index.engage(self.game)
                        self.yellow_replay_step_up(fiveodds - self.game.yellow_replay_counter.position)
    ...
    def orange_replay_step_up(self, number):
        if number >= 1:
            self.game.orange_replay_counter.step()
            number -= 1
            self.replay_step_up()
            if self.game.replays == 899:
                number = 0
            self.delay(name="orange_replay_step_up", delay=0.1, handler=self.orange_replay_step_up, param=number)
        else:
            self.game.search_index.disengage()
            self.cancel_delayed(name="orange_replay_step_up")
            self.search()
    ...
    def closed_search_relays(self, rivets, c):
        ...
        return (self.pos[rivets], card, corners, yellow, orange, red)
    ...
    def all_probability(self):
        ...
        if self.game.orange_odds.position < 3:
            self.orange_extra_step(1)
    ...
    def orange_extra_step(self, number):
        if number > 0:
            self.game.orange_odds.step()
            self.delay(name="display", delay=0, handler=graphics.playtime.display, param=self)
            self.game.coils.sounder.pulse()
            number -= 1
            self.delay(name="orange_extra_step", delay=0.1, handler=self.orange_extra_step, param=number)
    ...
    def orange_odds_probability(self):
        ...
        if self.game.orange_odds.position == 1:
            ...
            if self.game.orange_odds.position < 5:
                if self.game.cu:
                    return 1
            elif self.game.orange_odds.position == 5:
    ...
    def features_probability(self):
        ...
        if self.game.orange_odds.position > 1 and self.game.red_odds.position > 1 and self.game.yellow_odds.position > 1:
    ...
    def check_mixer4(self):
        ...
        g = self.game.red_odds.position
        y = self.game.yellow_odds.position
        r = self.game.orange_odds.position
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0, handler=graphics.playtime.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.playtime.display, param=self)
            s -= 1
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.playtime.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.playtime.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.playtime.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.playtime.display, param=self)
            s -= 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [167,257]
            image = pygame.image.load('playtime/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [241,290]
            image = pygame.image.load('playtime/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [346,298]
            image = pygame.image.load('playtime/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [431,264]
            image = pygame.image.load('playtime/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class Playtime(procgame.game.BasicGame):
    """ Palm Beach was the first game with Super Cards """
    def __init__(self, machine_type):
        super(Playtime, self).__init__(machine_type)
        ...
        #Odds stepper
        self.orange_odds = units.Stepper("orange_odds", 8, 'playtime')
        self.yellow_odds = units.Stepper("yellow_odds", 8, 'playtime')
        self.red_odds = units.Stepper("red_odds", 8, 'playtime')

        #Replay Counter
        self.orange_replay_counter = units.Stepper("orange_replay_counter", 1000)
        ...
        self.square_a = units.Stepper("square_a", 3, "playtime", "continuous")
        self.square_b = units.Stepper("square_b", 3, "playtime", "continuous")
        self.square_c = units.Stepper("square_c", 3, "playtime", "continuous")
        self.square_d = units.Stepper("square_d", 3, "playtime", "continuous")
        self.square_e = units.Stepper("square_e", 3, "playtime", "continuous")
...
game = Playtime(machine_type='pdb')
game.reset()
game.run_loop()
