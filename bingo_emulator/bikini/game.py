...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_blue_active(self, sw):
        ...
        if self.game.replays > 0 or self.game.switches.freeplay.is_active():
            # Commenting this out until I get a physical futurity button to press
...
#        self.cancel_delayed(name="blink_title")
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.bikini.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.bikini.display, param=self)
            s -= 1
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.bikini.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.bikini.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.bikini.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.bikini.display, param=self)
            s -= 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [167,257]
            image = pygame.image.load('bikini/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [241,290]
            image = pygame.image.load('bikini/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [346,298]
            image = pygame.image.load('bikini/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [431,264]
            image = pygame.image.load('bikini/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
