...
from bingo_emulator.graphics.bull_market import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.bull_market.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh bull_market")
        else:
            if self.game.ball_count.position >= 4:
    ...
    def sw_left_active(self, sw):
        if self.game.before_third.status == True or self.game.before_fourth.status == True:
            self.game.spotting.stepdown()
            if self.game.spotting.position == 0:
                ...
                if self.game.top_spots.status == True:
            ...
            if self.game.spotting.position == 3:
                if 16 in self.holes:
                    if self.game.switches.hole16.is_inactive():
                        self.holes.remove(16)
                if 20 in self.holes:
                ...
                if 22 in self.holes:
                    if self.game.switches.hole22.is_inactive():
                        self.holes.remove(22)
                if self.game.bottom_spots.status == True:
            ...
            if self.game.spotting.position == 5:
                if 16 in self.holes:
                    if self.game.switches.hole16.is_inactive():
                        self.holes.remove(16)
                if 20 in self.holes:
    ...
    def regular_play(self):
        ...
        self.game.alternator = not self.game.alternator
    ...
    def check_spotting(self):
        if self.game.selector.position == 13 or self.game.selector.position == 24 or self.game.selector.position == 44:
            pos = self.game.probability.position
            if self.game.before_fourth.status == False:
                if self.game.cu == False:
                    if pos == 3 or pos == 4 or pos == 9 or pos == 10:
                        self.game.before_third.engage(self.game)
                else:
                    if pos == 3 or pos == 4 or pos == 9 or pos == 10 or pos == 11 or pos == 5:
                        self.game.before_third.engage(self.game)
            if self.game.before_third.status == False:
                if self.game.cu == False:
                    if pos == 1 or pos == 2 or pos == 7 or pos == 8:
                        self.game.before_fourth.engage(self.game)
                else:
                    if pos == 1 or pos == 2 or pos == 7 or pos == 8 or pos == 12 or pos == 6:
                        self.game.before_fourth.engage(self.game)
        if self.game.alternator == 1:
            self.game.top_spots.engage(self.game)
        else:
            self.game.bottom_spots.engage(self.game)
        if self.game.before_third.status == True or self.game.before_fourth.status == True:
            if self.game.spotting.position == 0:
                if 15 in self.holes:
                    if self.game.switches.hole15.is_inactive():
...
##        self.cancel_delayed(name="blink_title")
        graphics.bull_market.display(self, 0)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.bull_market.reel1, graphics.bull_market.reel10, graphics.bull_market.reel100, graphics.bull_market.reel1000)
                self.game.coils.registerDown.pulse()
                number -= 1
                graphics.bull_market.display(self)
    ...
    def replay_step_up(self):
        if self.game.replays < 8999:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.bull_market.reel1, graphics.bull_market.reel10, graphics.bull_market.reel100, graphics.bull_market.reel1000)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
        ...
        if self.game.switches.left.is_inactive() and self.game.switches.right.is_inactive():
            if i[0] == 1 and self.game.card1_replay_counter.position > 0:
                ...
                if i[0] == 1 and self.game.card1_missed.status == True:
                    ...
                    graphics.bull_market.blink_double(self)
    ...
    def closed_search_relays(self, rivets, c1, c2, c3, s1, s2, s3):
        ...
        self.pos[77] = {3:1, 15:2, 25:3, 17:4, 13:5}
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [61,251]
            image = pygame.image.load('bull_market/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [218,251]
            image = pygame.image.load('bull_market/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [406,252]
            image = pygame.image.load('bull_market/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [529,266]
            image = pygame.image.load('bull_market/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class BullMarket(procgame.game.BasicGame):
    """ Bright Lights was the first bingo produced by Bally """
    def __init__(self, machine_type):
        super(BullMarket, self).__init__(machine_type)
        ...
        self.selector = units.Stepper("selector", 50)
        ...
        #Spots relays - provide maximum movement.
        self.top_spots = units.Relay("top_spots")
        self.bottom_spots = units.Relay("bottom_spots")
        ...
        #Spotting is not a normal spotting disc - it's actually a disc to help me keep track
        #of the currently spotted number.  This also provides a seamless interface for the
        #subsequent game where the spotted number feature is available
        self.spotting = units.Stepper("spotting", 5, "bull_market", "continuous")
        ...
        #Time Tree for spotted number feature
        self.before_third = units.Relay("before_third")
        self.before_fourth = units.Relay("before_fourth")

        self.returned = False

    def reset(self):
        super(BullMarket, self).reset()
...
game = BullMarket(machine_type='pdb')
game.reset()
game.run_loop()
