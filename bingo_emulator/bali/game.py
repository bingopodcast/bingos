...
from bingo_emulator.graphics.bali import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.bali.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh bali")
        else:
            if self.game.ball_count.position >= 4:
...
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.bali.reel1, graphics.bali.reel10, graphics.bali.reel100, graphics.bali.reel1000)
                self.game.coils.registerDown.pulse()
                number -= 1
                graphics.bali.display(self)
    ...
    def replay_step_up(self):
        if self.game.replays < 8999:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.bali.reel1, graphics.bali.reel10, graphics.bali.reel100, graphics.bali.reel1000)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
        ...
        if self.game.switches.left.is_inactive() and self.game.switches.right.is_inactive():
            if i[0] == 1 and self.game.card1_replay_counter.position > 0:
                ...
                if i[0] == 1 and self.game.card1_missed.status == True:
                    ...
                    graphics.bali.blink_double(self)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [30,266]
            image = pygame.image.load('bali/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [192,270]
            image = pygame.image.load('bali/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [330,272]
            image = pygame.image.load('bali/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [524,270]
            image = pygame.image.load('bali/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class Bali(procgame.game.BasicGame):
    """ Bali was the only run for a short time """
    def __init__(self, machine_type):
        super(Bali, self).__init__(machine_type)
