...
class MulticardBingo(procgame.game.Mode):
    ...
    def super_card_replay_step_up(self, number):
        ...
        if number >= 1:
            ...
            if self.game.replays == 899:
                number = 0
            self.delay(name="super_card_replay_step_up", delay=0.25, handler=self.super_card_replay_step_up, param=number)
        else:
            self.game.search_index.disengage()
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.monaco.both_animation([self, start + num])
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.monaco.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
