...
from bingo_emulator.graphics.fun_way import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def play_replays(self):
        if self.game.replays > 0 and self.game.selector.position < 6:
            ...
            self.delay(name="display", delay=0.2, handler=graphics.fun_way.display, param=self)
    ...
    def sw_startButton_active(self, sw):
        if self.game.replays > 0 or self.game.switches.freeplay.is_active():
            ...
            self.delay(name="display", delay=0.1, handler=graphics.fun_way.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh fun_way")
...
#        self.cancel_delayed(name="blink_title")
        graphics.fun_way.display(self, 0)
        self.tilt_actions()
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [6,456]
            image = pygame.image.load('fun_way/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [107,462]
            image = pygame.image.load('fun_way/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
...
class FunWay(procgame.game.BasicGame):
    """ Fun Way was the first Ohio Dime Game to automatically play off replays """
    def __init__(self, machine_type):
        super(FunWay, self).__init__(machine_type)
        ...
        self.spotting = units.Stepper("spotting", 49, "Fun Way", "continuous")
