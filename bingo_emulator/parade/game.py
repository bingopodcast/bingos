...
from bingo_emulator.graphics.parade import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.delay(name="display", delay=0.1, handler=graphics.parade.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh parade")
    ...
    def regular_play(self):
        ...
        self.game.mixer4.spin()
        self.game.reflex.decrease()
        print self.game.before_fourth.status
        ...
        if self.game.start.status == True:
            ...
            self.game.top_line.disengage()
            self.game.bottom_line.disengage()
            self.game.green_line.reset()
    ...
    def magic_squarea_reset(self, number):
        if number != 0:
            self.game.square_a.step()
            self.delay(name="display", delay=0, handler=graphics.parade.display, param=self)
...
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.parade.reel1, graphics.parade.reel10, graphics.parade.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.parade.reel1, graphics.parade.reel10, graphics.parade.reel100)
            ...
            for key in self.wipers:
                for number in self.holes:
                    if number == key:
                        ...
                        if self.game.selector.position >= 1:
                            if s >= 3:
                                self.find_winner(s, self.card, self.green, self.red)
                                break
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)

    def find_winner(self, relays, card, green, red):
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if relays == 3:
                if red:
                    ...
                    if self.game.green_line.position >= 5 and self.game.green_line.position < 10:
                        if self.game.card1_replay_counter.position < threeodds * 2:
                            self.game.search_index.engage(self.game)
                            self.card1_replay_step_up(threeodds * 2 - self.game.card1_replay_counter.position)
                    elif self.game.green_line.position == 10:
                        if self.game.card1_replay_counter.position < threeodds * 3:
                            self.game.search_index.engage(self.game)
                            self.card1_replay_step_up(threeodds * 3 - self.game.card1_replay_counter.position)
                    elif self.game.green_line.position == 15:
                        if self.game.card1_replay_counter.position < threeodds * 4:
                            self.game.search_index.engage(self.game)
                            self.card1_replay_step_up(threeodds * 4 - self.game.card1_replay_counter.position)
                    else:
    ...
    def closed_search_relays(self, rivets):
        ...
        if self.game.top_line.status == True:
            self.o1 = 6
            self.o2 = 7
            self.o3 = 13
            self.o4 = 18
        ...
        if self.game.bottom_line.status == True:
            self.u1 = 5
            self.u2 = 8
            self.u3 = 3
            self.u4 = 19
        ...
        self.pos[6] = {self.o1:1, self.p1:2, self.q1:3, 2:4, self.s1:5, self.t1:6, self.u1:7}
        self.pos[7] = {self.o2:1, self.p2:2, self.q2:3, 18:4, self.s2:5, self.t2:6, self.u2:7}
        self.pos[8] = {self.o3:1, 25:2, 24:3, 16:4, 13:5, 5:6, self.u3:7}
        self.pos[9] = {self.o4:1, self.p4:2, self.q4:3, 12:4, self.s4:5, self.t4:6, self.u4:7}
        self.pos[10] = {self.o5:1, self.p5:2, self.q5:3, 11:4, self.s5:5, self.t5:6, self.u5:7}
        self.pos[11] = {self.p1:1, self.q2:2, 16:3, self.s4:4, self.t5:5}
        self.pos[12] = {self.p5:1, self.q4:2, 16:3, self.s2:4, self.t1:5}
        self.pos[13] = {}
        self.pos[14] = {}
        self.pos[15] = {self.o1:1, self.o2:2, self.o3:3, self.o4:4, self.o5:5}
        self.pos[16] = {}
        self.pos[17] = {self.u1:1, self.u2:2, self.u3:3, self.u4:4, self.u5:5}
        self.pos[18] = {}
        self.pos[19] = {}
        self.pos[20] = {}
        self.pos[21] = {}
        ...
        return (self.pos[rivets], card, green, red)
    ...
    def odds_probability(self):
        ...
        if self.game.odds.position == 1:
            ...
            if self.game.odds.position == 2:
                ...
                if sd in range(8,11) or sd == 14 or sd in range(16,19) or sd == 21 or sd == 24 or sd in range(30,33) or sd in range(38,40) or sd == 41 or sd == 46:
                    return 1
                else:
                    return 0
            elif self.game.odds.position == 4:
                if sd ==6 or sd in range(8,11) or sd == 14 or sd in range(16,19) or sd in range(30,33) or sd == 42 or sd == 47 or sd == 0:
                    return 1
                else:
                    return 0
            elif self.game.odds.position == 5 or self.game.odds.position == 6:
                if sd == 2 or sd == 3 or sd == 12 or sd == 21 or sd == 28 or sd == 35 or sd == 36 or sd == 49:
                    return 1
                else:
                    return 0
            elif self.game.odds.position == 7:
    ...
    def check_mixer3(self):
        ...
        if self.game.cu:
            ...
            if self.game.red_line.position < 15:
                return 1
            elif self.game.green_line.position < 15:
                return 1
            else:
    ...
    def features_spotting(self):
        ...
        if sd == 4:
            ...
            self.step_red_line(10 - self.game.red_line.position)
        elif sd == 2:
            if self.game.cu:
        ...
        if sd == 5:
            self.step_green_line(15)
        elif sd == 7:
            self.step_green_line(5 - self.game.green_line.position)
        elif sd == 40:
            self.step_green_line(10 - self.game.green_line.position)
        elif sd == 21:
            if self.game.cu:
                if self.game.green_line.position < 5:
                    self.step_green_line(4 - self.game.green_line.position)
                elif self.game.green_line.position < 10:
                    self.step_green_line(9 - self.game.green_line.position)
                else:
                    self.step_green_line(14 - self.game.green_line.position)
        else:
            if self.game.cu:
                self.game.green_line.step()
        if sd == 37:
            if self.game.reflex.connected_rivet() == 4 or self.game.reflex.connected_rivet() == 3:
                self.step_ms(6 - self.game.magic_squares_feature.position)
        if sd == 11:
            self.step_ms(6 - self.game.magic_squares_feature.position)
        if sd == 4:
            ...
            self.step_ms(1)
        if sd == 27:
            self.game.top_line.engage(self.game)
        if sd == 16:
            self.game.bottom_line.engage(self.game)
        if sd == 31:
    ...
    def step_green_line(self, number):
        if number >= 1:
            self.game.green_line.step()
            number -= 1
            self.delay(name="display", delay=0.1, handler=graphics.parade.display, param=self)
            self.delay(name="step_green_line", delay=0.1, handler=self.step_green_line, param=number)
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0, handler=graphics.parade.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.parade.display, param=self)
            s -= 1
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.parade.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.parade.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.parade.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.parade.display, param=self)
            s -= 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [167,257]
            image = pygame.image.load('parade/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [241,290]
            image = pygame.image.load('parade/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [346,298]
            image = pygame.image.load('parade/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [431,264]
            image = pygame.image.load('parade/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class Parade(procgame.game.BasicGame):
    """ Palm Beach was the first game with Super Cards """
    def __init__(self, machine_type):
        super(Parade, self).__init__(machine_type)
        ...
        #Odds stepper
        self.odds = units.Stepper("odds", 8, 'parade')
        ...
        self.top_line = units.Relay("top_line")
        self.bottom_line = units.Relay("bottom_line")

        self.green_line = units.Stepper("green_line", 15)
        self.red_line = units.Stepper("red_line", 15)

        self.square_a = units.Stepper("square_a", 3, "parade", "continuous")
        self.square_b = units.Stepper("square_b", 3, "parade", "continuous")
        self.square_c = units.Stepper("square_c", 3, "parade", "continuous")
        self.square_d = units.Stepper("square_d", 3, "parade", "continuous")

        self.magic_squares_feature = units.Stepper("magic_squares_feature", 6)
