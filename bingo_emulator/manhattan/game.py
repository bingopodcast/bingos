...
from bingo_emulator.graphics.manhattan import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            self.scan_all()
        self.delay(name="display", delay=0.1, handler=graphics.manhattan.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh manhattan")
...
#        self.cancel_delayed(name="blink_title")
        graphics.manhattan.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.manhattan.reel1, graphics.manhattan.reel10, graphics.manhattan.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.manhattan.reel1, graphics.manhattan.reel10, graphics.manhattan.reel100)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [284,249]
            image = pygame.image.load('manhattan/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [229,275]
            image = pygame.image.load('manhattan/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [347,272]
            image = pygame.image.load('manhattan/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.manhattan.display, param=self)
        else:
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.manhattan.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.manhattan.display, param=self)
            s -= 1
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.manhattan.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.manhattan.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.manhattan.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.manhattan.display, param=self)
            s -= 1
...
class Manhattan(procgame.game.BasicGame):
    def __init__(self, machine_type):
        super(Manhattan, self).__init__(machine_type)
        ...
        #Odds stepper
        self.odds = units.Stepper("odds", 8, 'manhattan')
        ...
        self.spotted = units.Stepper("spotted", 10, "manhattan", "continuous")
...
game = Manhattan(machine_type='pdb')
game.reset()
game.run_loop()
