...
from bingo_emulator.graphics.miss_america_supreme import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == False:
            ...
            graphics.miss_america_supreme.display(self)
            self.animate_eb_scan([begin,self.game.spotting.movement_amount,self.game.spotting.movement_amount])

        self.delay(name="display", delay=0.1, handler=graphics.miss_america_supreme.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh miss_america_supreme")
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.miss_america_supreme.reel1, graphics.miss_america_supreme.reel10, graphics.miss_america_supreme.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.miss_america_supreme.reel1, graphics.miss_america_supreme.reel10, graphics.miss_america_supreme.reel100)
                                ...
                                if self.game.line5.position == 1:
                                    if self.num == 21:
                                        if 24 in self.holes:
                                            s += 1
                            if s >= 2:
                                self.find_winner(s, self.card, self.corners, self.color, self.stars)
                                break

    def find_winner(self, relays, card, corners, color, stars, red_letter_winner=0):
    ...
    def red_replay_step_up(self, number):
        ...
        if number >= 1:
            ...
            if self.game.replays == 899:
                number = 0
            self.delay(name="red_replay_step_up", delay=0.22, handler=self.red_replay_step_up, param=number)
        else:
            self.game.search_index.disengage()
            self.cancel_delayed(name="red_replay_step_up")
            self.search()
    ...
    def yellow_replay_step_up(self, number):
        ...
        if number >= 1:
            ...
            if self.game.replays == 899:
                number = 0
            self.delay(name="yellow_replay_step_up", delay=0.22, handler=self.yellow_replay_step_up, param=number)
        else:
            self.game.search_index.disengage()
            self.cancel_delayed(name="yellow_replay_step_up")
            self.search()
    ...
    def green_replay_step_up(self, number):
        ...
        if number >= 1:
            ...
            if self.game.replays == 899:
                number = 0
            self.delay(name="green_replay_step_up", delay=0.22, handler=self.green_replay_step_up, param=number)
        else:
            self.game.search_index.disengage()
            self.cancel_delayed(name="green_replay_step_up")
            self.search()
    ...
    def white_replay_step_up(self, number):
        ...
        if number >= 1:
            ...
            if self.game.replays == 899:
                number = 0
            self.delay(name="white_replay_step_up", delay=0.22, handler=self.white_replay_step_up, param=number)
        else:
            self.game.search_index.disengage()
            self.cancel_delayed(name="white_replay_step_up")
            self.search()
    ...
    def closed_search_relays(self, rivets, c):
        ...
        return (self.pos[rivets], card, corners, color, stars, rivets)
    ...
    def red_extra_step(self, number):
        if number > 0:
            self.game.red_odds.step()
            self.delay(name="display", delay=0, handler=graphics.miss_america_supreme.display, param=self)
            number -= 1
        ...
        if sd == 8:
            if self.game.rollovers.status == False:
                if self.game.before_fifth.status == False:
                    ...
                    self.game.before_fifth.engage(self.game)
                    self.game.sound.play('tilt')
        if sd in [3,4,6,22,23,26,27,37,38,39,46] or sd in range(8,13) or sd in range(16,20) or sd in range(29,35) or sd in range(42,45):
            if self.game.cu:
                if self.game.red_letter_two.status == False:
                    if self.game.red_letter_three.status == False:
                        self.game.red_letter_three.engage(self.game)
                        self.game.sound.play('tilt')
        if sd in [3,4,23,26,27,31,32,34,38,39,43,44] or sd in range(10,13) or sd in range(17,20):
            if self.game.red_letter_two.status == False:
                self.game.red_letter_three.disengage()
                self.game.red_letter_two.engage(self.game)
                self.game.sound.play('tilt')
        if sd in [12,24,38,39]:
            if self.game.green_corners.status == False:
                self.game.green_corners.engage(self.game)
                self.game.sound.play('tilt')
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.miss_america_supreme.both_animation([self, start + num])
    ...
    def animate_line(self, args):
        ...
        line = args[2]
        if num < 45:
            graphics.miss_america_supreme.line_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.miss_america_supreme.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class MissAmericaSupreme(procgame.game.BasicGame):
    """ Miss America Supreme added the red letter game """
    def __init__(self, machine_type):
        super(MissAmericaSupreme, self).__init__(machine_type)
        ...
        self.line1 = units.Stepper("line1", 3, "miss_america_supreme", "continuous")
        self.line2 = units.Stepper("line2", 3, "miss_america_supreme", "continuous")
        self.line3 = units.Stepper("line3", 3, "miss_america_supreme", "continuous")
        self.line4 = units.Stepper("line4", 3, "miss_america_supreme", "continuous")
        self.line5 = units.Stepper("line5", 3, "miss_america_supreme", "continuous")
    ...
    def reset(self):
        super(MissAmericaSupreme, self).reset()
...
game = MissAmericaSupreme(machine_type='pdb')
game.reset()
game.run_loop()
