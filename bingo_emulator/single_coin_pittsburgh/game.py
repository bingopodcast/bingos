...
from bingo_emulator.graphics.single_coin_pittsburgh import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        graphics.single_coin_pittsburgh.display(self)
    ...
    def play_replays(self):
        if self.game.selector.position < 6:
            ...
            self.delay(name="display", delay=0.2, handler=graphics.single_coin_pittsburgh.display, param=self)
    ...
    def sw_startButton_active(self, sw):
        if self.game.replays > 0 or self.game.switches.freeplay.is_active():
            self.game.tilt.disengage()
            self.regular_play()
            self.delay(name="display", delay=0.1, handler=graphics.single_coin_pittsburgh.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh single_coin_pittsburgh")
    ...
    def check_spotting(self):
        ...
        if self.game.spotting.position == 4:
        ...
        elif self.game.spotting.position == 24:
            if 21 not in self.holes:
                self.holes.append(21)
            ...
            if 16 not in self.holes:
                self.holes.append(16)
        elif self.game.spotting.position == 32:
            if 15 not in self.holes:
                self.holes.append(15)
            ...
            if 17 not in self.holes:
                self.holes.append(17)
        elif self.game.spotting.position == 44:
            if 21 not in self.holes:
                self.holes.append(21)
        elif self.game.spotting.position == 45:
            if 15 not in self.holes:
                self.holes.append(15)
...
#        self.cancel_delayed(name="blink_title")
        graphics.single_coin_pittsburgh.display(self, 0)
        self.tilt_actions()
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def find_winner(self, relays, card):
        if self.game.search_index.status == False and self.game.replays < 200:
            if card == 1:
                ...
                if relays == 4:
                    if self.game.card1_replay_counter.position < 36:
                        self.game.search_index.engage(self.game)
                        self.card1_replay_step_up(36 - self.game.card1_replay_counter.position)
                if relays == 5:
            ...
            if card == 2:
                ...
                if relays == 4:
                    if self.game.card2_replay_counter.position < 36:
                        self.game.search_index.engage(self.game)
                        self.card2_replay_step_up(36 - self.game.card2_replay_counter.position)
                if relays == 5:
            ...
            if card == 3:
                ...
                if relays == 4:
                    if self.game.card3_replay_counter.position < 36:
                        self.game.search_index.engage(self.game)
                        self.card3_replay_step_up(36 - self.game.card3_replay_counter.position)
                if relays == 5:
            ...
            if card == 4:
                ...
                if relays == 4:
                    if self.game.card4_replay_counter.position < 36:
                        self.game.search_index.engage(self.game)
                        self.card4_replay_step_up(36 - self.game.card4_replay_counter.position)
                if relays == 5:
            ...
            if card == 5:
                ...
                if relays == 4:
                    if self.game.card5_replay_counter.position < 36:
                        self.game.search_index.engage(self.game)
                        self.card5_replay_step_up(36 - self.game.card5_replay_counter.position)
                if relays == 5:
            ...
            if card == 6:
                ...
                if relays == 4:
                    if self.game.card6_replay_counter.position < 36:
                        self.game.search_index.engage(self.game)
                        self.card6_replay_step_up(36 - self.game.card6_replay_counter.position)
                if relays == 5:
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [6,456]
            image = pygame.image.load('single_coin_pittsburgh/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [107,462]
            image = pygame.image.load('single_coin_pittsburgh/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
...
class SingleCoinPittsburgh(procgame.game.BasicGame):
    def __init__(self, machine_type):
        super(SingleCoinPittsburgh, self).__init__(machine_type)
        ...
        self.spotting = units.Stepper("spotting", 49, "Single Coin Pittsburgh", "continuous")
    ...
    def reset(self):
        super(SingleCoinPittsburgh, self).reset()
...
game = SingleCoinPittsburgh(machine_type='pdb')
game.reset()
game.run_loop()
