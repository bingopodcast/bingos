...
from bingo_emulator.graphics.rio import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            graphics.rio.display(self)
            self.game.eb_play.disengage()
        else:
            self.game.tilt.disengage()
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.rio.display, param=self)
    ...
    def sw_trough8_closed(self, sw):
        if self.game.start.status == False:
            ...
            if self.game.letter_i.status == False:
                self.game.letter_i.engage(self.game)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh rio")
    ...
    def sw_redstar_active(self, sw):
        if self.game.red_star.status == True:
            ...
            if self.game.ball_count.position >= 5:
                self.search()
        if self.game.letter_o.status == False:
            self.game.letter_o.engage(self.game)
            self.game.sound.play('tilt')
            self.search()
        self.delay(name="display", delay=0.1, handler=graphics.rio.display, param=self)
    ...
    def sw_yellowstar_active(self, sw):
        ...
        if self.game.letter_r.status == False:
            self.game.letter_r.engage(self.game)
            self.game.sound.play('tilt')
            self.search()
        self.delay(name="display", delay=0.1, handler=graphics.rio.display, param=self)
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            ...
            if self.game.eb_play.status == True:
                ...
                self.game.letter_r.disengage()
                self.game.letter_i.disengage()
                self.game.letter_o.disengage()
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.rio.reel1, graphics.rio.reel10, graphics.rio.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.rio.reel1, graphics.rio.reel10, graphics.rio.reel100)
                    ...
                    if number == key:
                        ...
                        if self.game.selector.position >= self.card:
                            if s >= 2:
                                ...
                                break
        if self.game.letter_r.status == True and self.game.letter_i.status == True:
            self.find_winner(0, 1, 0, 0, 0, 0, 2)
        if self.game.letter_r.status == True and self.game.letter_i.status == True and self.game.letter_o.status == True:
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.rio.display, param=self)
        else:
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [1,3,12,14,17,21,24]:
                ...
                graphics.rio.animate_mixer1(self)
            if rivet in [2,6,10,11,20,23]:
                self.game.mixer2.step()
                graphics.rio.animate_mixer2(self)
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
                graphics.rio.animate_mixer3(self)
            graphics.rio.both_animation([self, start + num])
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
            graphics.rio.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Rio(procgame.game.BasicGame):
    """ Rio is the first game with Lite-a-Name """
    def __init__(self, machine_type):
        super(Rio, self).__init__(machine_type)
        ...
        self.reflex1 = units.Stepper("reflex1", 50, "Rio", "continuous")
        self.reflex2 = units.Stepper("reflex2", 50, "Rio")
        ...
        self.spotted = units.Stepper("spotted", 10, "rio", "continuous")
        ...
        self.letter_r = units.Relay("letter_r")
        self.letter_i = units.Relay("letter_i")
        self.letter_o = units.Relay("letter_o")
