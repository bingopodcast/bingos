...
from bingo_emulator.graphics.cabana import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            graphics.cabana.display(self)
            self.game.eb_play.disengage()
        else:
            self.game.sound.play_music('motor', -1)
            self.game.tilt.disengage()
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.cabana.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh cabana")
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.cabana.reel1, graphics.cabana.reel10, graphics.cabana.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.cabana.reel1, graphics.cabana.reel10, graphics.cabana.reel100)
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.cabana.display, param=self)
        else:
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [0,4,7,8,16,18]:
                ...
                graphics.cabana.animate_mixer4(self)
            if rivet in [1,3,12,14,17,21,24]:
                self.game.mixer1.step()
                graphics.cabana.animate_mixer1(self)
            if rivet in [2,6,10,11,20,23]:
                self.game.mixer2.step()
                graphics.cabana.animate_mixer2(self)
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
                graphics.cabana.animate_mixer3(self)
            graphics.cabana.both_animation([self, start + num])
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
            graphics.cabana.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Cabana(procgame.game.BasicGame):
    """ Cabana is the fourth United game with a standard Bally 25 hole playfield.  It's not _exactly_ the same, but close enough. """
    def __init__(self, machine_type):
        super(Cabana, self).__init__(machine_type)
        ...
        #Odds stepper
        self.odds = units.Stepper("odds", 8, 'Cabana')
        ...
        self.reflex1 = units.Stepper("reflex1", 50, "Cabana", "continuous")
        self.reflex2 = units.Stepper("reflex2", 50, "Cabana")
        ...
        self.spotted = units.Stepper("spotted", 6, "cabana", "continuous")
