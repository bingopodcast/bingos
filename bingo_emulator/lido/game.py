...
from bingo_emulator.graphics.lido import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.start.status == True:
            ...
            self.game.features.disengage()
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.lido.display, param=self)
    ...
    def sw_startButton_active(self, sw):
        ...
        self.cancel_delayed(name="ss_animation")
    ...
    def sw_blue_active(self, sw):
        ...
        if self.game.replays > 0 or self.game.switches.freeplay.is_active():
            ...
            if self.game.switches.drawer.is_inactive():
                if self.game.futurity.position > 0:
                    self.regular_play(self.game.futurity.position)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh lido")
        else:
            ...
            if self.game.ball_count.position >= 4:
                if self.game.search_index.status == False:
                    if self.game.magic_screen.position < 2:
                        ...
                        if orange_winner >= 2:
                            if self.game.pre_futurity == 0:
                                self.game.pre_futurity = self.game.futurity.position
                                self.add_futurity()
    ...
    def add_futurity(self):
        ...
        self.step_futurity(red_letter)

    def step_futurity(self, number):
        if number >= 1:
            self.game.futurity.step()
            number -= 1
            self.delay(name="display", delay=0.1, handler=graphics.lido.display, param=self)
            self.delay(name="step_futurity", delay=0.1, handler=self.step_futurity, param=number)
    ...
    def regular_play(self, red_letter=0):
        ...
        if self.game.ss.status == True:
            if self.game.magic_screen_feature.position >= 7:
                self.animate_ss_scan([begin,self.game.spotting.movement_amount,1])
            else:
                self.animate_both([begin,self.game.spotting.movement_amount,1])
        ...
        if self.game.start.status == True:
            ...
            if red_letter > 0:
                ...
                if red_letter == 1:
                ...
                elif red_letter == 3 or red_letter == 4:
                    self.red_extra_step(6)
                    self.yellow_extra_step(6)
                    self.step_magic_screen(7)
                    ...
                    self.step_sf(5)
                    self.delay(name="check_selection", delay=0.1, handler=self.check_selection)
                elif red_letter == 5 or red_letter == 6:
                    self.red_extra_step(5)
                    ...
                    self.step_sf(1)
                    self.delay(name="check_selection", delay=0.1, handler=self.check_selection)
                elif red_letter == 7 or red_letter == 8:
                    ...
                    self.step_sf(7)
                    self.delay(name="check_selection", delay=0.1, handler=self.check_selection)
                elif red_letter == 9 or red_letter == 10 or red_letter == 11:
                    self.red_extra_step(7)
                    self.yellow_extra_step(6)
                    self.step_magic_screen(9)
                    self.game.three_blue.engage(self.game)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.lido.reel1, graphics.lido.reel10, graphics.lido.reel100)
                self.game.coils.registerDown.pulse()
                number -= 1
                self.delay(name="display", delay=0, handler=graphics.lido.display, param=self)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.lido.reel1, graphics.lido.reel10, graphics.lido.reel100)
    ...
    def ss_probability(self):
        ...
        if sd in [5,6,8,9,13,18,23,27,28,30,38,39,44,47]:
            self.ss_spotting()
    ...
    def ss_spotting(self):
        if self.game.selection_feature.position in range(1,5):
            ...
            if self.game.mixer2.position in range(0,6) or self.game.mixer2.position in range(8,13) or self.game.mixer2.position in [14,16]:
                if self.game.cu:
                    if self.game.red_super_section.status == False:
                        self.game.red_super_section.engage(self.game)
                        self.game.sound.play('tilt')
    ...
    def animate_screen_right(self, args):
        ...
        num = args[1]
        if num >= 0:
            graphics.lido.screen_animation([self, num, "right"])
    ...
    def animate_screen_left(self, args):
        ...
        num = args[1]
        if num >= 0:
            graphics.lido.screen_animation([self, num, "left"])
            self.cancel_delayed(name="display")
            num = num - 1
    ...
    def animate_odds_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.lido.odds_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_features_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.lido.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_ss_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.lido.ss_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
            self.delay(name="ss_animation", delay=0.08, handler=self.animate_ss_scan, param=args)
        else:
            self.cancel_delayed(name="feature_animation")
            self.delay(name="display", delay=0.1, handler=graphics.lido.display, param=self)
            self.scan_ss()
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.lido.both_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.lido.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Lido(procgame.game.BasicGame):
    """ Lido has futurity and white button super section play """
    def __init__(self, machine_type):
        super(Lido, self).__init__(machine_type)
        ...
        self.cu = 1
        self.cam4 = units.Stepper("cam4", 3, "lido", "continuous")
        ...
        #Odds steppers
        self.red_odds = units.Stepper("red_odds", 8, 'lido')
        self.yellow_odds = units.Stepper("yellow_odds", 8, 'lido')
        self.green_odds = units.Stepper("green_odds", 8, 'lido')
        ...
        self.futurity = units.Stepper("futurity", 12)
