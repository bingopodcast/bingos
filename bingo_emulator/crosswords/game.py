...
from bingo_emulator.graphics.crosswords import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.crosswords.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh crosswords")
 ...
 #       self.cancel_delayed(name="blink_title")
        graphics.crosswords.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.crosswords.reel1, graphics.crosswords.reel10, graphics.crosswords.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 99:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.crosswords.reel1, graphics.crosswords.reel10, graphics.crosswords.reel100)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [182,299]
            image = pygame.image.load('crosswords/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [246,298]
            image = pygame.image.load('crosswords/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [294,298]
            image = pygame.image.load('crosswords/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [381,298]
            image = pygame.image.load('crosswords/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
        if title5 == 1:
            pos = [449,298]
            image = pygame.image.load('crosswords/assets/title5_on.png').convert_alpha()
            screen.blit(image, pos)
...
class Crosswords(procgame.game.BasicGame):
...
game = Crosswords(machine_type='pdb')
game.reset()
game.run_loop()
