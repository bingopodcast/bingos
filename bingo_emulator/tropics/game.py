...
from bingo_emulator.graphics.tropics import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            graphics.tropics.display(self)
            self.game.eb_play.disengage()
        else:
            self.game.tilt.disengage()
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.tropics.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh tropics")
            ...
            if self.game.before_fourth.status == True:
                if self.game.selection_feature_relay.status == True:
                    if self.game.spotted.position == 7:
                        self.step_eb(24 - self.game.extra_ball.position)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.tropics.reel1, graphics.tropics.reel10, graphics.tropics.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.tropics.reel1, graphics.tropics.reel10, graphics.tropics.reel100)
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.tropics.display, param=self)
        else:
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [1,3,12,14,17,21,24]:
                ...
                graphics.tropics.animate_mixer1(self)
            if rivet in [2,6,10,11,20,23]:
                self.game.mixer2.step()
                graphics.tropics.animate_mixer2(self)
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
                graphics.tropics.animate_mixer3(self)
            graphics.tropics.both_animation([self, start + num])
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
            graphics.tropics.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Tropics(procgame.game.BasicGame):
    """ Tropics is a United game with select-a-feature """
    def __init__(self, machine_type):
        super(Tropics, self).__init__(machine_type)
        ...
        #Odds stepper
        self.odds = units.Stepper("odds", 8, 'tropics')
        ...
        self.reflex1 = units.Stepper("reflex1", 50, "Tropics", "continuous")
        self.reflex2 = units.Stepper("reflex2", 50, "Tropics")
        ...
        self.spotted = units.Stepper("spotted", 10, "tropics", "continuous")
