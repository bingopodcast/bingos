...
from bingo_emulator.graphics.continental import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh continental")
        else:
            if self.game.ball_count.position >= 4:
                #self.game.sound.stop_music()
    ...
    def animate_dd(self, args):
        ...
        if self.game.dd1.status == True:
            self.game.dd1.disengage()
            dirty_rects.append(screen.blit(bg_gi, (546,636), pygame.Rect(546,636,55,24)))
        if self.game.dd2.status == True:
            self.game.dd2.disengage()
            dirty_rects.append(screen.blit(bg_gi, (605,636), pygame.Rect(605,636,55,24)))
        if self.game.dd3.status == True:
            self.game.dd3.disengage()
            dirty_rects.append(screen.blit(bg_gi, (665,636), pygame.Rect(665,636,55,24)))
        if self.game.dd4.status == True:
            self.game.dd4.disengage()
            dirty_rects.append(screen.blit(bg_gi, (546,681), pygame.Rect(546,681,55,24)))
        if self.game.dd5.status == True:
            self.game.dd5.disengage()
            dirty_rects.append(screen.blit(bg_gi, (605,681), pygame.Rect(605,681,55,24)))
        if self.game.dd6.status == True:
            self.game.dd6.disengage()
            dirty_rects.append(screen.blit(bg_gi, (665,681), pygame.Rect(665,681,55,24)))
        pygame.display.update(dirty_rects)
    ...
    def sw_replayReset_active(self, sw):
        ...
        self.holes = []
        graphics.continental.display(self, 0)
        self.tilt_actions()
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.continental.reel1, graphics.continental.reel10, graphics.continental.reel100, graphics.continental.reel1000)
            ...
            if self.game.switches.left.is_inactive() and self.game.switches.right.is_inactive():
                if i[0] == 1 and self.game.card1_replay_counter.position > 0:
                    ...
                    if i[0] == 1 and self.game.card1_missed.status == True:
                        ...
                        graphics.continental.blink_double(self)
    ...
    def check_double_probability(self):
        ...
        if pos in [1,4,5,6,8,10,11,15,16,17,19,21,22,24,25,27,29,30,33,34,39,40,44,46,48]:
            return 1
        elif self.game.probability.position == 3 or self.game.probability.position == 10:
            return 1
        else:
            return 0
...
class Continental(procgame.game.BasicGame):
    """ Continental was the first purely solid state bingo """
    def __init__(self, machine_type):
        super(Continental, self).__init__(machine_type)
...
game = Continental(machine_type='pdb')
game.reset()
game.run_loop()
