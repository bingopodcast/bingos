...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [284,249]
            image = pygame.image.load('rodeo_3/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [229,275]
            image = pygame.image.load('rodeo_3/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [347,272]
            image = pygame.image.load('rodeo_3/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.rodeo_3.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.rodeo_3.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.rodeo_3.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.rodeo_3.display, param=self)
            s -= 1
...
class Rodeo3(procgame.game.BasicGame):
    """ Rodeo3 is the second United game with a standard Bally 25 hole playfield.  It's not _exactly_ the same, but close enough. """
