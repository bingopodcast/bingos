...
from bingo_emulator.graphics.malibu_beach import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh malibu_beach")
        else:
...
#        self.cancel_delayed(name="blink_title")
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.malibu_beach.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.malibu_beach.display, param=self)
            s -= 1
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.malibu_beach.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.malibu_beach.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.malibu_beach.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.malibu_beach.display, param=self)
            s -= 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [167,257]
            image = pygame.image.load('malibu_beach/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [241,290]
            image = pygame.image.load('malibu_beach/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [346,298]
            image = pygame.image.load('malibu_beach/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [431,264]
            image = pygame.image.load('malibu_beach/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class MalibuBeach(procgame.game.BasicGame):
    """ Palm Beach was the first game with Super Cards """
    def __init__(self, machine_type):
        super(MalibuBeach, self).__init__(machine_type)
        ...
        #Odds steppers
        self.red_odds = units.Stepper("red_odds", 8, 'malibu_beach')
        self.yellow_odds = units.Stepper("yellow_odds", 8, 'malibu_beach')
        self.green_odds = units.Stepper("green_odds", 8, 'malibu_beach')
...
game = MalibuBeach(machine_type='pdb')
game.reset()
game.run_loop()
