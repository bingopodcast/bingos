...
from bingo_emulator.graphics.key_west import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.delay(name="display", delay=0.1, handler=graphics.key_west.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh key_west")
        if self.game.before_fourth.status == True:
            max_ball = 4
        elif self.game.after_fifth.status == True:
            max_ball = 6
        if self.game.select_a_score.status == True:
            red = self.game.red_odds.position
            yellow = self.game.yellow_odds.position
            self.game.red_odds.position = yellow
            self.game.yellow_odds.position = red
            if self.game.swapped == False:
                self.game.swapped = True
            else:
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            ...
            self.game.select_a_score.disengage()
            self.game.active = ""
            self.game.switched = False
    ...
    def magic_squarea_reset(self, number):
        if number != 0:
            self.game.square_a.step()
            self.delay(name="display", delay=0, handler=graphics.key_west.display, param=self)
...
#        self.cancel_delayed(name="blink_title")
        graphics.key_west.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.key_west.reel1, graphics.key_west.reel10, graphics.key_west.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.key_west.reel1, graphics.key_west.reel10, graphics.key_west.reel100)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def features_spotting(self):
        ...
        if ms <= 2:
            self.step_ms(1)
        if ms == 3:
            if sd == 3 or sd == 7 or sd == 30 or sd == 48:
                self.step_ms(1)
        if ms == 4:
            if sd == 1 or sd == 5 or sd == 6 or sd == 9 or sd == 21 or sd == 27 or sd == 34 or sd == 39 or sd == 44 or sd == 49:
                self.step_ms(1)
        if ms == 5:
            if sd == 20 or sd == 26 or sd == 28 or sd == 33 or sd == 38 or sd == 43:
                self.step_ms(1)
        if ms == 6:
            if sd == 0 or sd == 16:
                self.step_ms(1)
        ...
        if sd == 44:
            self.game.select_a_score.engage(self.game)
        if sd == 2 or sd == 10 or sd == 19 or sd ==  22 or sd == 24 or sd == 42:
    ...
    def check_rollover(self):
        if self.game.cu:
            ...
            self.game.active = "yellow"
        else:
            self.game.coils.redROLamp.enable()
            self.game.coils.yellowROLamp.disable()
            self.game.active = "red"

    def sw_redstar_active(self, sw):
        if self.game.rollovers.status == True:
            if self.game.active == "red":
                if self.game.after_fifth.status == False:
    ...
    def step_ms(self, number):
        if number >= 1:
            ...
            if self.game.magic_squares_feature.position == 6:
                ...
                if m3 == 2 or m3 == 5 or m3 == 8 or m3 == 11 or m3 == 13 or m3 == 14 or m3 == 19 or m3 == 23:
                    self.holes.append(18)
                else:
                    self.holes.append(2)
            number -= 1
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0, handler=graphics.key_west.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.key_west.display, param=self)
            s -= 1
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.key_west.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.key_west.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.key_west.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.key_west.display, param=self)
            s -= 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [167,257]
            image = pygame.image.load('key_west/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [241,290]
            image = pygame.image.load('key_west/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [346,298]
            image = pygame.image.load('key_west/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [431,264]
            image = pygame.image.load('key_west/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class KeyWest(procgame.game.BasicGame):
    """ Palm Beach was the first game with Super Cards """
    def __init__(self, machine_type):
        super(KeyWest, self).__init__(machine_type)
        ...
        #Odds stepper
        self.red_odds = units.Stepper("red_odds", 8, 'key_west')
        self.yellow_odds = units.Stepper("yellow_odds", 8, 'key_west')
        self.green_odds = units.Stepper("green_odds", 8, 'key_west')
        ...
        self.square_a = units.Stepper("square_a", 3, "key_west", "continuous")
        self.square_b = units.Stepper("square_b", 3, "key_west", "continuous")
        self.square_c = units.Stepper("square_c", 3, "key_west", "continuous")
        self.square_d = units.Stepper("square_d", 3, "key_west", "continuous")
        ...
        self.select_a_score = units.Relay("select_a_score")
