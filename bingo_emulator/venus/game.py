...
from bingo_emulator.graphics.venus import *
...
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.venus.reel1, graphics.venus.reel10, graphics.venus.reel100, graphics.venus.reel1000)
                self.game.coils.registerDown.pulse()
                number -= 1
                graphics.venus.display(self)
    ...
    def replay_step_up(self):
        if self.game.replays < 8999:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.venus.reel1, graphics.venus.reel10, graphics.venus.reel100, graphics.venus.reel1000)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
                ...
                if card == 4:
                    ...
                    if relays == 4:
                        if self.game.card4_replay_counter.position < 20:
                            self.game.search_index.engage(self.game)
                            self.game.sound.stop('search')
                            self.wait_for_input((4, 20))
                    if relays == 5:
                        if self.game.card4_replay_counter.position < 160:
                            self.game.search_index.engage(self.game)
                            self.game.sound.stop('search')
                            self.wait_for_input((4, 160))
                ...
                if card == 5:
                    ...
                    if relays == 4:
                        if self.game.card5_replay_counter.position < 20:
                            self.game.search_index.engage(self.game)
                            self.game.sound.stop('search')
                            self.wait_for_input((5, 20))
                    if relays == 5:
                        if self.game.card5_replay_counter.position < 180:
                            self.game.search_index.engage(self.game)
                            self.game.sound.stop('search')
                            self.wait_for_input((5, 180))
                ...
                if card == 6:
                    ...
                    if relays == 4:
                        if self.game.card6_replay_counter.position < 20:
                            self.game.search_index.engage(self.game)
                            self.game.sound.stop('search')
                            self.wait_for_input((6, 20))
                    if relays == 5:
                        if self.game.card6_replay_counter.position < 200:
                            self.game.search_index.engage(self.game)
                            self.game.sound.stop('search')
                            self.wait_for_input((6, 200))

    def wait_for_input(self, i):
        if self.game.switches.startButton.is_inactive() and self.game.switches.green.is_inactive():
            if i[0] == 1 and self.game.card1_replay_counter.position > 0:
                ...
                if i[0] == 1 and self.game.card1_missed.status == True:
                    ...
                    graphics.venus.blink_double(self)
        ...
        if self.game.switches.startButton.is_active():
            ...
            self.delay(name="display", delay=0.1, handler=graphics.venus.display, param=self)
        if self.game.switches.green.is_active():
            self.cancel_delayed(name="blink_double")
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [30,266]
            image = pygame.image.load('venus/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [192,270]
            image = pygame.image.load('venus/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [330,272]
            image = pygame.image.load('venus/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [524,270]
            image = pygame.image.load('venus/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class Venus(procgame.game.BasicGame):
    """ Bright Lights was the first bingo produced by Bally """
    def __init__(self, machine_type):
        super(Venus, self).__init__(machine_type)
