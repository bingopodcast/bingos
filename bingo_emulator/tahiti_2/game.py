...
from bingo_emulator.graphics.tahiti_2 import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.start.status == True:
            ...
            if self.game.coin.position < 40:
                self.scan_all()
        self.delay(name="display", delay=0.1, handler=graphics.tahiti_2.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh tahiti_2")
        else:
    ...
    def sw_left_active_for_2s(self, sw):
        ...
        graphics.tahiti_2.display(self)
...
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.tahiti_2.reel1, graphics.tahiti_2.reel10, graphics.tahiti_2.reel100)
                self.game.coils.registerDown.pulse()
                number -= 1
                self.delay(name="display", delay=0, handler=graphics.tahiti_2.display, param=self)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.tahiti_2.reel1, graphics.tahiti_2.reel10, graphics.tahiti_2.reel100)
...
#        self.cancel_delayed(name="blink_title")
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.tahiti_2.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.tahiti_2.display, param=self)
            s -= 1
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.tahiti_2.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.tahiti_2.display, param=self)
            s -= 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [167,257]
            image = pygame.image.load('tahiti_2/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [241,290]
            image = pygame.image.load('tahiti_2/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [346,298]
            image = pygame.image.load('tahiti_2/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [431,264]
            image = pygame.image.load('tahiti_2/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class Tahiti2(procgame.game.BasicGame):
    """ Palm Beach was the first game with Super Cards """
    def __init__(self, machine_type):
        super(Tahiti2, self).__init__(machine_type)
        ...
        #Odds steppers
        self.red_odds = units.Stepper("red_odds", 10, 'tahiti_2')
        self.yellow_odds = units.Stepper("yellow_odds", 10, 'tahiti_2')
        self.green_odds = units.Stepper("green_odds", 10, 'tahiti_2')
        self.blue_odds = units.Stepper("blue_odds", 10, 'tahiti_2')
        ...
        self.line1 = units.Stepper("line1", 3, "tahiti_2", "continuous")
        self.line2 = units.Stepper("line2", 1, "tahiti_2", "continuous")
        self.line3 = units.Stepper("line3", 1, "tahiti_2", "continuous")
        ...
        self.bc = units.Relay("bc")

        self.coin = units.Stepper("coin", 40, "tahiti_2")

        self.replays = 0
        self.returned = False

    def reset(self):
        super(Tahiti2, self).reset()
