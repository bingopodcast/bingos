...
from bingo_emulator.graphics.hole_in_one import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.hole_in_one.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh hole_in_one")
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            ...
            self.game.excellent.disengage()
            self.game.superior.disengage()
 ...
 #       self.cancel_delayed(name="blink_title")
        graphics.hole_in_one.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.hole_in_one.reel1, graphics.hole_in_one.reel10, graphics.hole_in_one.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.hole_in_one.reel1, graphics.hole_in_one.reel10, graphics.hole_in_one.reel100)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def find_winner(self, relays, card):
        if self.game.search_index.status == False and self.game.replays < 9:
            if card == 1:
                if relays == 3:
                    if self.game.card1_replay_counter.position < 1:
                        self.game.search_index.engage(self.game)
                        if self.game.excellent.status == False and self.game.superior.status == False:
                            self.game.good.engage(self.game)
                        self.card1_replay_step_up(1 - self.game.card1_replay_counter.position)
                if relays == 4:
                    if self.game.card1_replay_counter.position < 2:
                        self.game.search_index.engage(self.game)
                        self.game.good.disengage()
                        if self.game.superior.status == False:
                            self.game.excellent.engage(self.game)                        
                ...
                if relays == 5:
                    if self.game.card1_replay_counter.position < 3:
                        ...
                        self.game.good.disengage()
                        self.game.excellent.disengage()
                        self.game.superior.engage(self.game)
                        self.card1_replay_step_up(3 - self.game.card1_replay_counter.position)
            if card == 2:
                if relays == 3:
                    if self.game.card2_replay_counter.position < 1:
                        self.game.search_index.engage(self.game)
                        if self.game.excellent.status == False and self.game.superior.status == False:
                            self.game.good.engage(self.game)
                        self.card2_replay_step_up(1 - self.game.card2_replay_counter.position)
                if relays == 4:
            ...
            if card == 3:
                if relays == 3:
                    if self.game.card3_replay_counter.position < 1:
                        self.game.search_index.engage(self.game)
                        if self.game.excellent.status == False and self.game.superior.status == False:
                            self.game.good.engage(self.game)
                        self.card3_replay_step_up(1 - self.game.card3_replay_counter.position)
                if relays == 4:
            ...
            if card == 4:
                if relays == 3:
                    if self.game.card4_replay_counter.position < 1:
                        self.game.search_index.engage(self.game)
                        if self.game.excellent.status == False and self.game.superior.status == False:
                            self.game.good.engage(self.game)
                        self.card4_replay_step_up(1 - self.game.card4_replay_counter.position)
                if relays == 4:
            ...
            if card == 5:
                if relays == 3:
                    if self.game.card5_replay_counter.position < 1:
                        self.game.search_index.engage(self.game)
                        if self.game.excellent.status == False and self.game.superior.status == False:
                            self.game.good.engage(self.game)
                        self.card5_replay_step_up(1 - self.game.card5_replay_counter.position)
                if relays == 4:
            ...
            if card == 6:
                if relays == 3:
                    if self.game.card6_replay_counter.position < 1:
                        self.game.search_index.engage(self.game)
                        if self.game.excellent.status == False and self.game.superior.status == False:
                            self.game.good.engage(self.game)
                        self.card6_replay_step_up(1 - self.game.card6_replay_counter.position)
                if relays == 4:
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [182,299]
            image = pygame.image.load('hole_in_one/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [246,298]
            image = pygame.image.load('hole_in_one/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [294,298]
            image = pygame.image.load('hole_in_one/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [381,298]
            image = pygame.image.load('hole_in_one/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
        if title5 == 1:
            pos = [449,298]
            image = pygame.image.load('hole_in_one/assets/title5_on.png').convert_alpha()
            screen.blit(image, pos)
...
class HoleInOne(procgame.game.BasicGame):
    """ Bright Lights was the first bingo produced by Bally """
    def __init__(self, machine_type):
        super(HoleInOne, self).__init__(machine_type)
        ...
        self.excellent = units.Relay("excellent")
        self.superior = units.Relay("superior")
...
game = HoleInOne(machine_type='pdb')
game.reset()
game.run_loop()
