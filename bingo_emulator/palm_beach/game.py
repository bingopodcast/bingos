...
from bingo_emulator.graphics.palm_beach import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        if self.game.all_advantages.status == True:
            ...
            self.game.features.disengage()
            self.scan_all()
        self.delay(name="display", delay=0.1, handler=graphics.palm_beach.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh palm_beach")
...
#        self.cancel_delayed(name="blink_title")
        graphics.palm_beach.display(self)
        self.tilt_actions()
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.palm_beach.reel1, graphics.palm_beach.reel10, graphics.palm_beach.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.palm_beach.reel1, graphics.palm_beach.reel10, graphics.palm_beach.reel100)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def find_winner(self, relays, card, horizontal, vertical, diagonal, hodds, vodds, dodds, fourodds, fiveodds, corners, supercard):
        if self.game.search_index.status == False and self.game.replays < 899:
            if card == 1:
                if relays == 3:
                    if supercard == 1 and not corners:
                    ...
                    elif supercard == 2 and not corners:
                ...
                if relays == 4:
                    if supercard == 1 and corners:
                        if self.game.corners.status == True:
                            if self.game.super_card.position >= 4:
                                if self.game.corners_replay_counter.position < 400:
                                    self.game.search_index.engage(self.game)
                                    self.corners_replay_step_up(400 - self.game.corners_replay_counter.position)
                    elif supercard == 2 and corners:
                        if self.game.corners.status == True:
                            if self.game.super_card.position == 8:
                                if self.game.corners_replay_counter.position < 400:
                                    self.game.search_index.engage(self.game)
    ...
    def closed_search_relays(self, rivets, c):
        ...
        self.pos[23] = {19:1, 20:2, 21:3, 22:4}
        ...
        self.pos[32] = {12:1, 14:2, 18:3, 23:4}
        self.pos[33] = {}
        self.pos[34] = {}
        self.pos[35] = {}
        self.pos[36] = {}
        ...
        if rivets in range(11,13):
            diagonal = True
        if rivets in range(15,24):
            sc = 1
        if rivets == 23:
            corners = True
        if rivets in range(24,33):
            sc = 2
        if rivets == 32:
            corners = True
        if rivets == 13:
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.palm_beach.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.palm_beach.display, param=self)
            s -= 1
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.palm_beach.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.palm_beach.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.palm_beach.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.palm_beach.display, param=self)
            s -= 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [167,257]
            image = pygame.image.load('palm_beach/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [241,290]
            image = pygame.image.load('palm_beach/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [346,298]
            image = pygame.image.load('palm_beach/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [431,264]
            image = pygame.image.load('palm_beach/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class PalmBeach(procgame.game.BasicGame):
    """ Palm Beach was the first game with Super Cards """
    def __init__(self, machine_type):
        super(PalmBeach, self).__init__(machine_type)
        ...
        #Odds stepper
        self.odds = units.Stepper("odds", 10, 'palm_beach')
...
game = PalmBeach(machine_type='pdb')
game.reset()
game.run_loop()
