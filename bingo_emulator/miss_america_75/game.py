...
from bingo_emulator.graphics.miss_america_75 import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            ...
            self.game.red_missed.disengage()
            self.game.yellow_missed.disengage()
            self.game.green_missed.disengage()
            self.game.white_missed.disengage()
            self.game.red_double.disengage()
            self.game.yellow_double.disengage()
            self.game.green_double.disengage()
            self.game.white_double.disengage()
            self.game.red_regular.disengage()
            self.game.yellow_regular.disengage()
            self.game.green_regular.disengage()
            self.game.white_regular.disengage()
...
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.miss_america_75.reel1, graphics.miss_america_75.reel10, graphics.miss_america_75.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 8999:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.miss_america_75.reel1, graphics.miss_america_75.reel10, graphics.miss_america_75.reel100, graphics.miss_america_75.reel1000)
            ...
            for key in self.wipers:
                for number in self.holes:
                    if number == key:
                        ...
                        if self.game.selector.position >= 1:
                            if self.color == "red":
                                g = self.game.red_missed.status
                            if self.color == "green":
                                g = self.game.green_missed.status
                            if self.color == "yellow":
                                g = self.game.yellow_missed.status
                            if self.color == "white":
                                g = self.game.white_missed.status
                            if g == False:
                                if s >= 3:
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
                ...
                if corners and self.game.corners.status == True:
                    if self.game.corners.status == True:
                        if self.game.red_replay_counter.position < fiveodds:
                            self.game.search_index.engage(self.game)
                            self.game.sound.stop('search')
                            self.wait_for_input(("red", fiveodds))
                else:
                    if not corners:
                        if self.game.selector.position >= 1:
                            if color == "red":
    ...
    def wait_for_input(self, i):
        if self.game.switches.left.is_inactive() and self.game.switches.right.is_inactive():
            if i[0] == "red" and self.game.red_replay_counter.position > 0:
                if self.game.red_double.status == True:
                    ...
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                    self.game.blink = 0
                    self.red_replay_step_up(i[1] - self.game.red_replay_counter.position)
            elif i[0] == "green" and self.game.green_replay_counter.position > 0:
                if self.game.green_double.status == True:
                    self.cancel_delayed(name="blink_double")
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                    self.game.blink = 0
                    ...
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                    self.game.blink = 0
                    self.green_replay_step_up(i[1] - self.game.green_replay_counter.position)
            elif i[0] == "yellow" and self.game.yellow_replay_counter.position > 0:
                if self.game.yellow_double.status == True:
                    self.cancel_delayed(name="blink_double")
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                    self.game.blink = 0
                    ...
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                    self.game.blink = 0
                    self.yellow_replay_step_up(i[1] - self.game.yellow_replay_counter.position)
            elif i[0] == "white" and self.game.white_replay_counter.position > 0:
                if self.game.white_double.status == True:
                    self.cancel_delayed(name="blink_double")
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                    self.game.blink = 0
                    ...
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                    self.game.blink = 0
                    self.white_replay_step_up(i[1] - self.game.white_replay_counter.position)
            else:
                if i[0] == "red" and self.game.red_missed.status == True:
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                elif i[0] == "green" and self.game.green_missed.status == True:
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                elif i[0] == "yellow" and self.game.yellow_missed.status == True:
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                elif i[0] == "white" and self.game.white_missed.status == True:
                    self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
                else:
                    graphics.miss_america_75.blink_double(self)
        ...
        if self.game.switches.left.is_active():
            ...
            if i[0] == "red":
                self.red_replay_step_up(i[1] - self.game.red_replay_counter.position)
                self.game.red_regular.engage(self.game)
            elif i[0] == "green":
                self.green_replay_step_up(i[1] - self.game.green_replay_counter.position)
                self.game.green_regular.engage(self.game)
            elif i[0] == "yellow":
                self.yellow_replay_step_up(i[1] - self.game.yellow_replay_counter.position)
                self.game.yellow_regular.engage(self.game)
            elif i[0] == "white":
                self.white_replay_step_up(i[1] - self.game.white_replay_counter.position)
                self.game.white_regular.engage(self.game)
            self.delay(name="display", delay=0.1, handler=graphics.miss_america_75.display, param=self)
    ...
    def red_extra_step(self, number):
        if number > 0:
            self.game.red_odds.step()
            self.game.coils.sounder.pulse()
            self.delay(name="display", delay=0, handler=graphics.miss_america_75.display, param=self)
            number -= 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [167,257]
            image = pygame.image.load('miss_america_75/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [241,290]
            image = pygame.image.load('miss_america_75/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [346,298]
            image = pygame.image.load('miss_america_75/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [431,264]
            image = pygame.image.load('miss_america_75/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
...
class MissAmerica75(procgame.game.BasicGame):
    """ Palm Beach was the first game with Super Cards """
    def __init__(self, machine_type):
        super(MissAmerica75, self).__init__(machine_type)
        ...
        self.red_regular = units.Relay("red_regular")
        self.green_regular = units.Relay("green_regular")
        self.yellow_regular = units.Relay("yellow_regular")
        self.white_regular = units.Relay("white_regular")
        ...
        self.white_double = units.Relay("white_double")

        #Missed Relays
        self.red_missed = units.Relay("red_missed")
        self.green_missed = units.Relay("green_missed")
        self.yellow_missed = units.Relay("yellow_missed")
        self.white_missed = units.Relay("white_missed")


        # Select-a-spot setup
        self.selection_feature = units.Stepper("selection_feature", 9)
        self.line1 = units.Stepper("line1", 3, "miss_america_75", "continuous")
        self.line2 = units.Stepper("line2", 3, "miss_america_75", "continuous")
        self.line3 = units.Stepper("line3", 3, "miss_america_75", "continuous")
        self.line4 = units.Stepper("line4", 3, "miss_america_75", "continuous")
        self.line5 = units.Stepper("line5", 3, "miss_america_75", "continuous")
    ...
    def reset(self):
        super(MissAmerica75, self).reset()
...
game = MissAmerica75(machine_type='pdb')
game.reset()
game.run_loop()
