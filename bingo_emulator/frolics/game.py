...
class MulticardBingo(procgame.game.Mode):
    ...
    def check_mixer(self):
        if self.game.reflex.connected_rivet(5) == 5:
            return 1
        elif self.game.mixer1.position == 1 and self.game.reflex.connected_rivet(5) >= 4:
            return 1
        elif self.game.mixer1.position == 2 and self.game.reflex.connected_rivet(5) >= 3:
            return 1
        elif (self.game.mixer1.position == 3 or self.game.mixer1.position == 4) and self.game.reflex.connected_rivet(5) >= 2:
            return 1
        elif (self.game.mixer1.position == 5 or self.game.mixer1.position == 6) and self.game.reflex.connected_rivet(5) >= 1:
            return 1
        else:
            return 0
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [182,299]
            image = pygame.image.load('frolics/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [246,298]
            image = pygame.image.load('frolics/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [294,298]
            image = pygame.image.load('frolics/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [381,298]
            image = pygame.image.load('frolics/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
        if title5 == 1:
            pos = [449,298]
            image = pygame.image.load('frolics/assets/title5_on.png').convert_alpha()
            screen.blit(image, pos)
...
class Frolics(procgame.game.BasicGame):
    ...
    def __init__(self, machine_type):
        ...
        self.cu = 1
        self.mixer1 = units.Mixer("mixer_1", 6)
        self.mixer2 = units.Mixer("mixer2", 6)
