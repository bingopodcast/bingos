...
from bingo_emulator.graphics.safari import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.start.status == True:
            ...
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.safari.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh safari")
        else:
    ...
    def regular_play(self, red_letter=0):
        ...
        if self.game.start.status == True:
            ...
            if red_letter > 0:
                ...
                #OK WINNER
                self.holes = []
                self.game.animal_feature.position = 12
    ...
    def tilt_actions(self):
        ...
        self.game.all_advantages.engage(game)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.safari.reel1, graphics.safari.reel10, graphics.safari.reel100, graphics.safari.reel1000)
                self.game.coils.registerDown.pulse()
                number -= 1
                self.delay(name="display", delay=0, handler=graphics.safari.display, param=self)
    ...
    def replay_step_up(self):
        if self.game.replays < 8999:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.safari.reel1, graphics.safari.reel10, graphics.safari.reel100, graphics.safari.reel1000)
        ...
        if self.game.special_odds.position < 2:
            ...
            if r == 1:
                if self.game.special_odds.position == 2:
                    if m5 == 12:
                        self.game.special_odds.step()
                if self.game.special_odds.position == 3:
                    if m5 in [5,18]:
                        self.game.special_odds.step()
                if self.game.special_odds.position == 4:
                    if m5 == 12:
                        self.game.special_odds.step()
                if self.game.special_odds.position == 5:
                    if m5 in [5,18]:
                        self.game.special_odds.step()
    ...
    def animal_up(self, number):
        if number >= 1:
            self.game.animal_feature.step()
            number -= 1
            self.delay(name="display", delay=0.1, handler=graphics.safari.display, param=self)
            self.delay(name="animal_up", delay=0.1, handler=self.animal_up, param=number)

    def animal_down(self, number):
        if number >= 1:
            self.game.animal_feature.stepdown()
            number -= 1
            self.delay(name="display", delay=0.1, handler=graphics.safari.display, param=self)
            self.delay(name="animal_down", delay=0.1, handler=self.animal_down, param=number)
    ...
    def find_special_winner(self):
        ...
        if self.game.search_index.status == False:
            if self.game.animal_feature.position in [6,7,15,18]:
                if self.game.special_replay_counter.position < lionodds:
                    self.game.search_index.engage(self.game)
                    self.special_replay_step_up(lionodds - self.game.special_replay_counter.position)
            if self.game.animal_feature.position in [3,21]:
                if self.game.special_replay_counter.position < rhinoodds:
                    self.game.search_index.engage(self.game)
                    self.special_replay_step_up(rhinoodds - self.game.special_replay_counter.position)
            if self.game.animal_feature.position in [0,1,23,24]:
                if self.game.special_replay_counter.position < giraffeodds:
                    self.game.search_index.engage(self.game)
                    self.special_replay_step_up(giraffeodds - self.game.special_replay_counter.position)
    ...
    def special_replay_step_up(self, number):
        ...
        if number >= 1:
            ...
            self.game.special_odds.reset()
            self.game.search_index.disengage()
            self.delay(name="display", delay=0.25, handler=graphics.safari.display, param=self)
        ...
        if self.game.reflex.connected_rivet() == 0:
            ...
            if mix1 == 3:
                ...
                if self.game.red_odds.position > 0:
                    self.scan_features()
        if self.game.reflex.connected_rivet() == 1 and (mix1 in [0,2,4,7,10,12,14,16,17,18,20,22]):
            self.scan_odds()
            if self.game.red_odds.position > 0:
                self.scan_features()
        elif self.game.reflex.connected_rivet() == 2 and (mix1 in [5,21,23]):
            self.scan_odds()
            if self.game.red_odds.position > 0:
                self.scan_features()
        elif self.game.reflex.connected_rivet() == 3 and (mix1 in [13,15,19]):
            self.scan_odds()
            if self.game.red_odds.position > 0:
                self.scan_features()
        elif self.game.reflex.connected_rivet() == 4 and (mix1 in [8,9,11]):
            self.scan_odds()
            if self.game.red_odds.position > 0:
                self.scan_features()
        else:
    ...
    def check_mixer2(self, sel):
        ...
        if ml < 4:
            ...
            if mix2 in [0,23,6,15,16,4,13,14]:
                return 1
        elif sf == 5:
            if mix2 in [2,3]:
                return 1
    ...
    def check_mixer4(self):
        ...
        if m4 in [7,9,15,19,22]:
            if self.game.blue_odds.position in [1,2,3,4]:
                self.game.mixer4_relay.engage(self.game)
        if m4 in [1,17,19]:
            if self.game.blue_odds.position == 5:
                self.game.mixer4_relay.engage(self.game)
        if m4 in [2,6,8,10,11]:
            if self.game.blue_odds.position == 6:
                self.game.mixer4_relay.engage(self.game)
        if m4 in [3,4]:
            if self.game.blue_odds.position == 7:
                self.game.mixer4_relay.engage(self.game)
        if m4 in [5,12,16,18]:
            if self.game.blue_odds.position == 8:
                self.game.mixer4_relay.engage(self.game)
        if m4 in [13,14]:
            if self.game.blue_odds.position == 9:
                self.game.mixer4_relay.engage(self.game)
        if m4 == 20:
            if self.game.blue_odds.position == 10:
                self.game.mixer4_relay.engage(self.game)
        if self.game.blue_odds.position == 10:
            if self.game.cu:
                ...
                if self.game.double_green.status == False:
                    self.game.mixer4_relay.engage(self.game)
        if m4 in [3,5,7]:
            if self.game.yellow_odds.position in [1,2,3,4]:
                self.game.mixer4_relay.engage(self.game)
        if m4 in [1,9]:
            if self.game.yellow_odds.position == 5:
                self.game.mixer4_relay.engage(self.game)
        if m4 in [12,13,15,21]:
            if self.game.yellow_odds.position == 6:
                self.game.mixer4_relay.engage(self.game)
        if m4 in [4,23]:
            if self.game.yellow_odds.position == 7:
        ...
        if self.game.yellow_odds.position == 10:
            if self.game.double_yellow.status == False:
                self.game.mixer4_relay.engage(self.game)
        if m4 in [7,11,15]:
            if self.game.red_odds.position in [1,2,3,4]:
                self.game.mixer4_relay.engage(self.game)
        if m4 in [9,13]:
            if self.game.red_odds.position == 5:
                self.game.mixer4_relay.engage(self.game)
        ...
        if self.game.eb_play.status == False:
            if self.game.cu:
                if self.game.green_odds.position >= 3:
                    if self.game.mixer1.position in [6,10,18,22]:
                        if self.check_mixer2(self.game.selection_feature.position) == True:
                            if self.game.triple.status == False:
                         ...
                         if self.game.cu or self.game.triple.status == False:
                            if self.game.green_odds.position >= 3:
                                if m3 in [1,2,5,9,13,18,21,22]:
                                    ...
                                    if seld.game.double_green.status == False:
                                        self.game.double_green.engage(self.game)
                                        self.game.sound.play('tilt')
    ...
    def animate_special(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.safari.special_animation([self, start + num])
    ...
    def animate_odds_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.safari.odds_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_line1(self, args):
        ...
        line = args[2]
        if num < 55:
            graphics.safari.line1_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
    ...
    def animate_line2(self, args):
        ...
        line = args[2]
        if num < 55:
            graphics.safari.line2_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
            args = [self.game,num,line]
    ...
    def animate_line3(self, args):
        ...
        line = args[2]
        if num < 55:
            graphics.safari.line3_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
            args = [self.game,num,line]
    ...
    def animate_features_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.safari.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.safari.both_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.safari.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Safari(procgame.game.BasicGame):
    """ Safari is the only game with the Animal feature """
    def __init__(self, machine_type):
        super(Safari, self).__init__(machine_type)
        ...
        #Odds steppers
        self.red_odds = units.Stepper("red_odds", 10, 'safari')
        self.yellow_odds = units.Stepper("yellow_odds", 10, 'safari')
        self.green_odds = units.Stepper("green_odds", 10, 'safari')
        self.blue_odds = units.Stepper("blue_odds", 10, 'safari')
        ...
        self.line1 = units.Stepper("line1", 3, "safari", "continuous")
        self.line2 = units.Stepper("line2", 1, "safari", "continuous")
        self.line3 = units.Stepper("line3", 1, "safari", "continuous")
        ...
        self.special_odds = units.Stepper("special_odds", 9, 'safari')
        ...
        self.giraffe = units.Relay("giraffe")
        self.rhino = units.Relay("rhino")
        self.lion = units.Relay("lion")

        self.animal_feature = units.Stepper("animal_feature", 24)
