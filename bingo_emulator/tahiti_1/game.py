...
from bingo_emulator.graphics.tahiti_1 import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            graphics.tahiti_1.display(self)
            self.game.eb_play.disengage()
        else:
            self.game.tilt.disengage()
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.tahiti_1.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh tahiti_1")
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.tahiti_1.reel1, graphics.tahiti_1.reel10, graphics.tahiti_1.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.tahiti_1.reel1, graphics.tahiti_1.reel10, graphics.tahiti_1.reel100)
    ...
    def find_winner(self, relays, card, corners, supercard, left_specialcard, right_specialcard):
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.tahiti_1.display, param=self)
        else:
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [0,4,7,8,16,18]:
                ...
                graphics.tahiti_1.animate_mixer4(self)
            if rivet in [1,3,12,14,17,21,24]:
                self.game.mixer1.step()
                graphics.tahiti_1.animate_mixer1(self)
            if rivet in [2,6,10,11,20,23]:
                self.game.mixer2.step()
                graphics.tahiti_1.animate_mixer2(self)
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
                graphics.tahiti_1.animate_mixer3(self)
            graphics.tahiti_1.both_animation([self, start + num])
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            ...
            if rivet in [5,9,13,15,19,22]:
                self.game.mixer3.step()
            graphics.tahiti_1.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Tahiti_1(procgame.game.BasicGame):
    """ Tahiti is a United game. """
    def __init__(self, machine_type):
        super(Tahiti_1, self).__init__(machine_type)
        ...
        self.reflex1 = units.Stepper("reflex1", 50, "Tahiti_1", "continuous")
        self.reflex2 = units.Stepper("reflex2", 50, "Tahiti_1")
        ...
        self.spotted = units.Stepper("spotted", 10, "tahiti_1", "continuous")
...
game = Tahiti_1(machine_type='pdb')
game.reset()
game.run_loop()
