...
class MulticardBingo(procgame.game.Mode):
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.brazil.both_animation([self, start + num])
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.brazil.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def features_probability(self):
        ...
        if sd == 35:
            if self.game.corners.status == False:
                ...
                self.game.sound.play('tilt')
        if sd in [1,3,5]:
            if self.game.row3.status == False:
                if self.game.row1.status == True:
                    self.game.row3.engage(self.game)
                    self.game.sound.play('tilt')
                    self.check_spot()
        if sd in [2,4,6]:
            if self.game.row2.status == False:
                if self.game.row1.status == True:
        ...
        if self.game.selection_feature.position == 2:
            if self.game.row1.status == False:
