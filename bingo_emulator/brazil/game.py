...
from bingo_emulator.graphics.brazil import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            self.scan_all()
        self.delay(name="display", delay=0.1, handler=graphics.brazil.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh brazil")
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            ...
            self.game.row1.disengage()
            self.game.row2.disengage()
            self.game.row3.disengage()
...
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.brazil.reel1, graphics.brazil.reel10, graphics.brazil.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.brazil.reel1, graphics.brazil.reel10, graphics.brazil.reel100)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [284,249]
            image = pygame.image.load('brazil/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [229,275]
            image = pygame.image.load('brazil/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [347,272]
            image = pygame.image.load('brazil/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.brazil.display, param=self)
        else:
    ...
    def features_probability(self):
        ...
        if sd == 1 or sd == 3 or sd == 5:
            if self.game.row1.status == True:
                self.game.row3.engage(self.game)
        if sd == 2 or sd == 4 or sd == 6:
            if self.game.row1.status == True:
                self.game.row2.engage(self.game)
        if sd == 0 or sd == 25:
        ...
        if self.game.selection_feature.position == 2:
            self.game.row1.engage(self.game)
    ...
    def odds_probability(self):
        ...
        if m3 == 2 or m3 == 1:
            ...
            if m3 == 8 or m3 == 15 or m3 == 5 or m3 == 11 or m3 == 4:
                return 1
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.brazil.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.brazil.display, param=self)
            s -= 1
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.brazil.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.brazil.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.brazil.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.brazil.display, param=self)
            s -= 1
...
class Brazil(procgame.game.BasicGame):
    def __init__(self, machine_type):
        super(Brazil, self).__init__(machine_type)
        ...
        #Odds stepper
        self.odds = units.Stepper("odds", 8, 'brazil')
        ...
        self.row1 = units.Relay("row1")
        self.row2 = units.Relay("row2")
        self.row3 = units.Relay("row3")
        self.select_spots = units.Relay("select_spots")
        self.spotted = units.Stepper("spotted", 4, "brazil", "continuous")
