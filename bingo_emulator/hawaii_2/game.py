...
from bingo_emulator.graphics.hawaii_2 import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.start.status == True or (self.game.x_feature.position == 6 and self.game.ball_count.position == 2):
            ...
            self.game.features.disengage()
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.hawaii_2.display, param=self)
    ...
    def sw_blue_active(self, sw):
        ...
        if self.game.start.status == True or (self.game.x_feature.position == 6 and self.game.ball_count.position == 2):
            ...
            self.delay(name="display", delay=0.1, handler=graphics.hawaii_2.display, param=self)
        if self.game.replays > 0 or self.game.switches.freeplay.is_active() and (self.game.ball_count.position == 0 or (self.game.x_feature.position == 6 and self.game.ball_count.position == 2)):
            if self.game.start.status == False and self.game.x_feature.position < 6:
        ...
        if self.game.ball_count.position >= 1:
            if self.game.score_select.position in [4,5,6,7]:
                if self.game.ball_count.position < 2:
                    self.game.color_selector.step()
                    self.delay(name="display", delay=0.1, handler=graphics.hawaii_2.display, param=self)
            if self.game.score_select.position == 8:
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh hawaii_2")
        else:
    ...
    def regular_play(self, red_letter=0):
        ...
        if self.game.x_feature.position == 6 and self.game.ball_count.position <= 2:
            return
        else:
            if self.game.start.status == True:
                ...
                graphics.hawaii_2.display(self)
                self.check_lifter_status()
            else:
                if red_letter > 0:
                    if self.game.switches.shutter.is_inactive():
                    ...
                    self.game.score_select.reset()
                    self.game.timer.reset()
                    self.step_scores(self.game.color_selector.position)
    ...
    def step_scores(self, number):
        if number != 0:
            self.game.color_selector.step()
            self.check_lifter_status()
            number = self.game.color_selector.position
            self.delay(name="display", delay=0.1, handler=graphics.hawaii_2.display, param=self)
            self.delay(name="step_scores", delay=0.1, handler=self.step_scores, param=number)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.hawaii_2.reel1, graphics.hawaii_2.reel10, graphics.hawaii_2.reel100, graphics.double_up.reel1000)
                self.game.coils.registerDown.pulse()
                number -= 1
                self.delay(name="display", delay=0, handler=graphics.hawaii_2.display, param=self)
    ...
    def replay_step_up(self):
        if self.game.replays < 8999:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.hawaii_2.reel1, graphics.hawaii_2.reel10, graphics.hawaii_2.reel100, graphics.hawaii_2.reel1000)
    ...
    def find_winner(self, relays, red, yellow, green, blue, stars, ok_winner=0, red_letter_winner=0):
        if self.game.search_index.status == False and self.game.replays < 8999:
            ...
            if self.game.double_red.status == True:
                if self.game.color_selector.position == 1:
                    redthreeodds = redthreeodds * 4
                    redfourodds = redfourodds * 4
                    redfiveodds = redfiveodds * 4
                else:
                    redthreeodds = redthreeodds * 2
                    redfourodds = redfourodds * 2
                    redfiveodds = redfiveodds * 2
            else:
                if self.game.color_selector.position > 0:
                    if self.game.color_selector.position == 1:
                        redthreeodds = redthreeodds * 2
                        redfourodds = redfourodds * 2
                        redfiveodds = redfiveodds * 2
                    else:
                        redthreeodds = redthreeodds * 0.5
                        redfourodds = redfourodds * 0.5
                        redfiveodds = redfiveodds * 0.5
            if self.game.double_yellow.status == True:
                if self.game.color_selector.position == 2:
                    yellowthreeodds = yellowthreeodds * 4
                    yellowfourodds = yellowfourodds * 4
                    yellowfiveodds = yellowfiveodds * 4
                else:
                ...
                if self.game.color_selector.position > 0:
                    if self.game.color_selector.position == 2:
                        ...
                        yellowthreeodds = yellowthreeodds * 0.5
                        yellowfourodds = yellowfourodds * 0.5
                        yellowfiveodds = yellowfiveodds * 0.5
            if self.game.double_green.status == True:
                if self.game.color_selector.position == 3:
                    greenthreeodds = greenthreeodds * 4
                    greenfourodds = greenfourodds * 4
                    greenfiveodds = greenfiveodds * 4
                else:
                ...
                if self.game.color_selector.position > 0:
                    if self.game.color_selector.position == 3:
                        ...
                        greenthreeodds = greenthreeodds * 0.5
                        greenfourodds = greenfourodds * 0.5
                        greenfiveodds = greenfiveodds * 0.5
            if self.game.double_blue.status == True:
                if self.game.color_selector.position == 4:
                    bluethreeodds = bluethreeodds * 4
                    bluefourodds = bluefourodds * 4
                    bluefiveodds = bluefiveodds * 4
                else:
                ...
                if self.game.color_selector.position > 0:
                    if self.game.color_selector.position == 4:
                        ...
                        bluethreeodds = bluethreeodds * 0.5
                        bluefourodds = bluefourodds * 0.5
                        bluefiveodds = bluefiveodds * 0.5
    ...
    def check_double_trips(self):
        ...
        if self.game.cu or self.game.selection_feature.position < 9:
            if sd in [5,6,18,19,20,21,36,37,41,42,43,44]:
                self.step_score_select(1)
            if sd in [12,13,14,25,38]:
                self.step_score_select(4)
    ...
    def features_spotting(self):
        sd = self.game.spotting.position
        if self.game.x_feature.position == 0:
            self.game.x_feature.step()
        else:
            if self.game.yellow_odds.position in [8,9,10]:
                if sd in [0,47]:
                    self.game.x_feature.step()
    ...
    def step_score_select(self, number):
        if number >= 1:
            self.game.score_select.step()
            number -= 1
            self.delay(name="display", delay=0.1, handler=graphics.hawaii_2.display, param=self)
            self.delay(name="step_sc", delay=0.1, handler=self.step_score_select, param=number)
    ...
    def animate_odds_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.hawaii_2.odds_animation([self, start + num])
    ...
    def animate_line1(self, args):
        ...
        line = args[2]
        if num < 55:
            graphics.hawaii_2.line1_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
    ...
    def animate_line2(self, args):
        ...
        line = args[2]
        if num < 55:
            graphics.hawaii_2.line2_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
            args = [self.game,num,line]
    ...
    def animate_line3(self, args):
        ...
        line = args[2]
        if num < 55:
            graphics.hawaii_2.line3_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
            args = [self.game,num,line]
    ...
    def animate_features_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.hawaii_2.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.hawaii_2.both_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.hawaii_2.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Hawaii2(procgame.game.BasicGame):
    """ Hawaii was the follow on to Double-Up and allowed for Stop and Shop """
    def __init__(self, machine_type):
        super(Hawaii2, self).__init__(machine_type)
        ...
        #Odds steppers
        self.red_odds = units.Stepper("red_odds", 10, 'hawaii_2')
        self.yellow_odds = units.Stepper("yellow_odds", 10, 'hawaii_2')
        self.green_odds = units.Stepper("green_odds", 10, 'hawaii_2')
        self.blue_odds = units.Stepper("blue_odds", 10, 'hawaii_2')

        self.x_feature = units.Stepper("x_feature", 6, 'hawaii_2')
        ...
        self.score_select = units.Stepper("score_select", 8)

        self.line1 = units.Stepper("line1", 3, "hawaii_2", "continuous")
        self.line2 = units.Stepper("line2", 1, "hawaii_2", "continuous")
        self.line3 = units.Stepper("line3", 1, "hawaii_2", "continuous")
        ...
        self.color_selector = units.Stepper("color_selector", 4, "hawaii_2", "continuous")

        self.replays = 0
        self.returned = False

    def reset(self):
        super(Hawaii2, self).reset()
