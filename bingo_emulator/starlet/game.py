...
from bingo_emulator.graphics.starlet import *
...
class MulticardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.eb_play.status == True and self.game.tilt.status == False:
            ...
            self.regular_play()
            self.scan_all()
        self.delay(name="display", delay=0.1, handler=graphics.starlet.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh starlet")
        else:
            max_ball = 4
    ...
    def regular_play(self):
        ...
        if self.game.start.status == True:
            ...
            self.game.horizontal_replay_counter.reset()
            ...
            self.game.super_horizontal.disengage()
...
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.starlet.reel1, graphics.starlet.reel10, graphics.starlet.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.starlet.reel1, graphics.starlet.reel10, graphics.starlet.reel100)
                ...
                for number in self.holes:
                    if number == key:
                        ...
                        if self.game.selector.position >= self.card:
                            if s >= 2:
                                self.find_winner(s, self.card, self.corners, self.horizontal)
                                break
        if self.game.name.position == 6:
            self.find_winner(0, 1, 0, 0, 1)
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)

    def find_winner(self, relays, card, corners, horizontal, name=0):
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if relays == 3:
                if not corners:
                    if horizontal:
                        if self.game.super_horizontal.status == True:
                            if self.game.horizontal.position >= 5:
                                if self.game.horizontal_replay_counter.position < fourodds:
                                    self.game.search_index.engage(self.game)
                                    self.horizontal_replay_step_up(fourodds - self.game.horizontal_replay_counter.position)
                            else:
                                 if self.game.horizontal_replay_counter.position < threeodds:
                                    self.game.search_index.engage(self.game)
                                    self.horizontal_replay_step_up(threeodds - self.game.horizontal_replay_counter.position)
                        else:
                            if self.game.horizontal.position >= 5:
                                if self.game.horizontal_replay_counter.position < fourodds:
                                    self.game.search_index.engage(self.game)
                ...
                if corners:
                    ...
                    if not corners:
                        if horizontal:
                            if self.game.super_horizontal.status == True:
                                if self.game.horizontal.position >= 5:
                                    if self.game.horizontal_replay_counter.position < fiveodds:
                                        self.game.search_index.engage(self.game)
                                        self.horizontal_replay_step_up(fiveodds - self.game.horizontal_replay_counter.position)
                                else:
                                     if self.game.horizontal_replay_counter.position < fourodds:
                                        self.game.search_index.engage(self.game)
                                        self.horizontal_replay_step_up(fourodds - self.game.horizontal_replay_counter.position)
    ...
    def horizontal_replay_step_up(self, number):
        if number >= 1:
            self.game.horizontal_replay_counter.step()
            number -= 1
            self.replay_step_up()
            if self.game.replays == 899:
                number = 0
            self.delay(name="horizontal_replay_step_up", delay=0.1, handler=self.horizontal_replay_step_up, param=number)
        else:
            self.game.search_index.disengage()
            self.cancel_delayed(name="horizontal_replay_step_up")
            self.search()
    ...
    def closed_search_relays(self, rivets):
        ...
        if rivets in range(15,20):
            horizontal = True
        if rivets == 28:
            corners = True

        return (self.pos[rivets], card, corners, horizontal)
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [284,249]
            image = pygame.image.load('starlet/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [229,275]
            image = pygame.image.load('starlet/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [347,272]
            image = pygame.image.load('starlet/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
...
#        self.delay(name="blink_title", delay=3, handler=self.blink_title)
    ...
    def scan_all(self):
        ...
        if self.game.odds.position < 1:
            self.game.odds.step()
            self.delay(name="display", delay=0, handler=graphics.starlet.display, param=self)
        else:
            if self.game.odds.position < 2:
                self.game.odds.step()
    ...
    def features_probability(self):
        ...
        if self.game.mixer1.position == 1:
            self.step_selection(6 - self.game.selection_feature.step())
        if sd == 41:
            self.game.before_fourth.disengage()
        ...
        if self.game.horizontal.position < 3:
            self.game.horizontal.step()
        if self.game.horizontal.position >= 3:
            if sd == 2
                self.game.horizontal.step()
        if sd == 20:
            self.game.super_horizontal.engage(self.game)
        if sd == 14:
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.starlet.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.starlet.display, param=self)
            s -= 1
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.starlet.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.starlet.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.starlet.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.starlet.display, param=self)
            s -= 1
...
class Starlet(procgame.game.BasicGame):
    def __init__(self, machine_type):
        super(Starlet, self).__init__(machine_type)
        ...
        self.roto = units.Stepper("roto", 7, 'starlet', "continuous")

        #Odds stepper
        self.odds = units.Stepper("odds", 8, 'starlet')
        
        #Replay Counter
        self.card1_replay_counter = units.Stepper("card1_replay_counter", 300)
        self.horizontal_replay_counter = units.Stepper("horizontal_replay_counter", 300)
        ...
        self.spotted = units.Stepper("spotted", 10, "starlet", "continuous")
        ...
        self.horizontal = units.Stepper("horizontal", 5)
        self.super_horizontal = units.Relay("super_horizontal")
