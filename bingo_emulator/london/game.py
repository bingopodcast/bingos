...
#        self.delay(name="blink_title", delay=1, handler=self.blink_title)
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.london.reel1, graphics.london.reel10, graphics.london.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.london.reel1, graphics.london.reel10, graphics.london.reel100)
        ...
        if self.game.start.status == False:
            ...
            if self.game.replays > 0 or self.game.switches.freeplay.is_active():
                if self.game.start.status == False:
                    ...
                    self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
                    self.regular_play()
                    self.game.special_reflex.decrease()
                    self.scan_special()
                    self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
...
#        self.cancel_delayed(name="blink_title")
    ...
    def special_replay_step_up(self, number):
        if number >= 1:
            ...
            if self.game.replays == 8999:
                number = 0
            self.delay(name="special_replay_step_up", delay=0.1, handler=self.special_replay_step_up, param=number)
        else:
            self.game.special_game.reset()
            self.game.special_odds.reset()
            self.game.search_index.disengage()
    ...
    def animate_odds_scan(self, s):
        if s > 1:
            self.delay(name="odds_animation", delay=0.1, handler=graphics.london.odds_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
            s -= 1
    ...
    def animate_feature_scan(self, s):
        if s > 1:
            self.delay(name="feature_animation", delay=0.1, handler=graphics.london.feature_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
            s -= 1
    ...
    def animate_eb_scan(self, s):
        if s > 1:
            self.delay(name="eb_animation", delay=0.1, handler=graphics.london.eb_animation, param=s)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
            s -= 1
    ...
    def blink_title(self):
        ...
        if title1 == 1:
            pos = [167,257]
            image = pygame.image.load('london/assets/title1_on.png').convert_alpha()
            screen.blit(image, pos)
        if title2 == 1:
            pos = [241,290]
            image = pygame.image.load('london/assets/title2_on.png').convert_alpha()
            screen.blit(image, pos)
        if title3 == 1:
            pos = [346,298]
            image = pygame.image.load('london/assets/title3_on.png').convert_alpha()
            screen.blit(image, pos)
        if title4 == 1:
            pos = [431,264]
            image = pygame.image.load('london/assets/title4_on.png').convert_alpha()
            screen.blit(image, pos)
