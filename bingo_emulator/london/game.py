...
from bingo_emulator.graphics.london import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.start.status == True:
            ...
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh london")
        else:
    ...
    def check_selection(self):
        if self.game.mystic_lines.position >= 4 or self.game.three_red_letter.status == True or self.game.two_red_letter.status == True:
    ...
    def regular_play(self, red_letter=0):
        ...
        self.cancel_delayed(name="blink")
        self.cancel_delayed(name="blink_triangle")
        self.cancel_delayed(name="blink_circle")
        self.cancel_delayed(name="blink_square")
        ...
        if self.game.special.status == True:
            self.animate_special([begin,self.game.spotting.movement_amount,1])
        ...
        if self.game.start.status == True:
            ...
            if red_letter > 0:
                ...
                self.game.special_replay_counter.reset()
                self.game.special_odds.reset()
                self.game.twin_number.reset()
                self.game.special_game.reset()
                self.game.missed.disengage()
                ...
                self.game.green_odds.reset()
                self.game.blue_odds.reset()
                self.game.eleven.disengage()
                self.game.seventeen.disengage()
                self.game.white1.disengage()
                self.game.pink1.disengage()
                self.game.orange1.disengage()
                self.game.white2.disengage()
                self.game.pink2.disengage()
                self.game.orange2.disengage()
                self.game.triangle.disengage()
                self.game.square.disengage()
                self.game.circle.disengage()
                self.game.special_color.reset()
                self.game.yellow_star.disengage()
                self.game.red_star.disengage()
                self.game.start.engage(self.game)
                self.game.ball_count.reset()
    ...
    def sw_hole2_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(2)
            self.check_special(2)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole3_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(3)
            self.check_special(3)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole4_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(4)
            self.check_special(4)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole5_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(5)
            self.check_special(5)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole6_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(6)
            self.check_special(6)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole7_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(7)
            self.check_special(7)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole8_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(8)
            self.check_special(8)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole9_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(9)
            self.check_special(9)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole10_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(10)
            self.check_special(10)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole11_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(11)
            self.check_special(11)
            if self.game.eleven.status == True:
    ...
    def sw_hole12_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            ...
            self.check_special(12)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole13_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(13)
            self.check_special(13)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole14_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(14)
            self.check_special(14)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole15_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(15)
            self.check_special(15)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole16_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(16)
            self.check_special(16)
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)

    def sw_hole17_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.holes.append(17)
            self.check_special(17)
            if self.game.seventeen.status == True:
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.london.reel1, graphics.london.reel10, graphics.london.reel100, graphics.london.reel1000)
                self.game.coils.registerDown.pulse()
                number -= 1
                self.delay(name="display", delay=0, handler=graphics.london.display, param=self)
    ...
    def replay_step_up(self):
        if self.game.replays < 8999:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.london.reel1, graphics.london.reel10, graphics.london.reel100, graphics.london.reel1000)
    ...
    def scan_special(self):
        self.check_lifter_status()
        m5 = self.game.mixer5.position
        self.check_color()
        if self.game.special_odds.position < 2:
            self.game.special_odds.step()
        else:
            r = self.check_special_reflex()
            if r == 1:
                if self.game.special_odds.position in [2,3,4,5]:
                    if m5 in [5,18]:
                        self.game.special_odds.step()
                if self.game.special_odds.position in [6,7,8]:
                    if m5 in [0,1,4,6,9,10,13,16,19,20]:
                        self.game.special_odds.step()
        self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
            
    def check_color(self):
        if self.game.special_reflex.connected_rivet() in [4,3,2,1]:
            self.game.special_color.step()
        if self.game.special_color.position in [2,4]:
            self.game.special_color.step()
        if self.game.special_color.position == 6:
            self.game.special_color.step()

    def check_special_reflex(self):
        if self.game.special_reflex.connected_rivet() == 0:
            #Worst position for special_reflex - requires mixer1 to be in the three liberal positions for the connection of the wires bypassing the special_reflex.
            return 0
        if self.game.special_reflex.connected_rivet() == 1 and self.game.cu:
            return 1
        elif self.game.special_reflex.connected_rivet() == 2 and self.game.cu:
            return 1
        elif self.game.special_reflex.connected_rivet() == 3 and self.game.cu:
            return 1
        elif self.game.special_reflex.connected_rivet() == 4:
            return 1

    def check_special(self, num):
        if self.game.ball_count.position <= 3:
            if self.game.circle.status == False:
                if num in [1,6,7,11,13,16,19]:
                    self.game.circle.engage(self.game)
                    self.game.sound.play('tilt')
            else:
                if num in [1,6,7,11,13,16,19]:
                    self.game.missed.engage(self.game)
                    self.game.sound.play('tilt')
            if self.game.square.status == False:
                if num in [2,4,8,10,14,15,20]:
                    self.game.square.engage(self.game)
                    self.game.sound.play('tilt')
            else:
                if num in [2,4,8,10,14,15,20]:
                    self.game.missed.engage(self.game)
                    self.game.sound.play('tilt')
            if self.game.triangle.status == False:
                if num in [3,5,9,12,17]:
                    self.game.triangle.engage(self.game)
                    self.game.sound.play('tilt')
            else:
                if num in [3,5,9,12,17]:
                    self.game.missed.engage(self.game)
                    self.game.sound.play('tilt')
        if self.game.ball_count.position == 3:
            if self.game.triangle.status == True and self.game.circle.status == True and self.game.square.status == True:
                self.game.special_game.step()
                self.game.special_color.step()

        if self.game.special_game.position == 1:
            if self.game.special_color.position in [0,4,9,12]:
                self.game.white1.engage(self.game)
            elif self.game.special_color.position in [1,3,5,7,8,11,13]:
                self.game.pink1.engage(self.game)
            elif self.game.special_color.position in [2,6,10,14]:
                self.game.orange1.engage(self.game)

        if self.game.ball_count.position == 4:
            if num != 0:
                if self.game.white1.status == True:
                    if num in [2,4,8,10,14,15,20]:
                        self.game.special_game.step()
                        self.game.special_color.step()
                    else:
                        self.game.missed.engage(self.game)
                        self.game.sound.play('tilt')
                elif self.game.pink1.status == True:
                    if num in [3,5,9,12,17]:
                        self.game.special_game.step()
                        self.game.special_color.step()
                    else:
                        self.game.missed.engage(self.game)
                        self.game.sound.play('tilt')
                elif self.game.orange1.status == True:
                    if num in [1,6,7,11,13,16,19]:
                        self.game.special_game.step()
                        self.game.special_color.step()
                    else:
        ...
        if self.game.special_game.position == 2:
            if self.game.special_color.position in [0,4,9,12]:
                self.game.white2.engage(self.game)
                self.game.sound.play('tilt')
            elif self.game.special_color.position in [1,3,5,7,8,11,13]:
                self.game.pink2.engage(self.game)
                self.game.sound.play('tilt')
            elif self.game.special_color.position in [2,6,10,14]:
                self.game.orange2.engage(self.game)
                self.game.sound.play('tilt')

        if self.game.ball_count.position == 5:
            if num != 0:
                if self.game.white2.status == True:
                    if num in [2,4,8,10,14,15,20]:
                        self.game.special_game.step()
                        self.game.special_color.step()
                    else:
                        self.game.missed.engage(self.game)
                        self.game.sound.play('tilt')
                elif self.game.pink2.status == True:
                    if num in [3,5,9,12,17]:
                        self.game.special_game.step()
                        self.game.special_color.step()
                    else:
                        self.game.missed.engage(self.game)
                        self.game.sound.play('tilt')
                elif self.game.orange2.status == True:
                    if num in [1,6,7,11,13,16,19]:
                        self.game.special_game.step()
                        self.game.special_color.step()
                    else:
        ...
        if self.game.missed.status == True:
            self.game.special_odds.reset()
            self.game.special_game.reset()
            self.game.special_color.reset()
        self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
    ...
    def search(self):
        ...
        for i in range(0, 6):
            ...
            if self.game.special_game.position >= 1 and self.game.missed.status == False:
                self.find_special_winner()

    def find_special_winner(self):
        if self.game.special_odds.position == 1:
            threeodds = 2
            fourodds = 6
            fiveodds = 18
        elif self.game.special_odds.position == 2:
            threeodds = 4
            fourodds = 12
            fiveodds = 36
        elif self.game.special_odds.position == 3:
            threeodds = 6
            fourodds = 18
            fiveodds = 54
        elif self.game.special_odds.position == 4:
            threeodds = 9
            fourodds = 27
            fiveodds = 81
        elif self.game.special_odds.position == 5:
            threeodds = 12
            fourodds = 36
            fiveodds = 108
        elif self.game.special_odds.position == 6:
            threeodds = 18
            fourodds = 54
            fiveodds = 162
        elif self.game.special_odds.position == 7:
            threeodds = 24
            fourodds = 72
            fiveodds = 216
        elif self.game.special_odds.position == 8:
            threeodds = 36
            fourodds = 108
            fiveodds = 327
        elif self.game.special_odds.position == 9:
            threeodds = 48
            fourodds = 144
            fiveodds = 432

        if self.game.missed.status == False:
            if self.game.search_index.status == False:
                if self.game.special_game.position == 1:
                    if self.game.special_replay_counter.position < threeodds:
                        self.game.search_index.engage(self.game)
                        self.special_replay_step_up(threeodds - self.game.special_replay_counter.position)
                if self.game.special_game.position == 2:
                    if self.game.special_replay_counter.position < fourodds:
                        self.game.search_index.engage(self.game)
                        self.special_replay_step_up(fourodds - self.game.special_replay_counter.position)
                if self.game.special_game.position == 3:
                    if self.game.special_replay_counter.position < fiveodds:
                        self.game.search_index.engage(self.game)
                        self.special_replay_step_up(fiveodds - self.game.special_replay_counter.position)
        ...
        if self.game.search_index.status == False and self.game.replays < 8999:
            ...
            if self.game.double_red.status == True:
                if self.game.triple.status == True:
                    redthreeodds = redthreeodds * 3
                    redfourodds = redfourodds * 3
                    redfiveodds = redfiveodds * 3
                else:
            ...
            if self.game.double_yellow.status == True:
                if self.game.triple.status == True:
                    yellowthreeodds = yellowthreeodds * 3
                    yellowfourodds = yellowfourodds * 3
                    yellowfiveodds = yellowfiveodds * 3
                else:
            ...
            if self.game.double_green.status == True:
                if self.game.triple.status == True:
                    greenthreeodds = greenthreeodds * 3
                    greenfourodds = greenfourodds * 3
                    greenfiveodds = greenfiveodds * 3
                else:
            ...
            if self.game.double_blue.status == True:
                if self.game.triple.status == True:
                    bluethreeodds = bluethreeodds * 3
                    bluefourodds = bluefourodds * 3
                    bluefiveodds = bluefiveodds * 3
                else:
    ...
    def special_replay_step_up(self, number):
        self.game.sound.stop('search')
        if number >= 1:
            self.game.special_replay_counter.step()
            number -= 1
            self.replay_step_up()
            if self.game.replays == 8999:
                number = 0
            self.delay(name="special_replay_step_up", delay=0.25, handler=self.special_replay_step_up, param=number)
        else:
            self.game.special_game.reset()
            self.game.special_odds.reset()
            self.game.search_index.disengage()
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
            self.cancel_delayed(name="special_replay_step_up")
            self.search()
    ...
    def scan_odds(self):
        if self.game.odds_only.status == True:
            ...
            if self.game.all_advantages.status == True:
                if self.game.yellow_odds.position in [0,1,2] or self.game.red_odds.position in [0,1,2] or self.game.green_odds.position in [0,1,2] or self.game.blue_odds.position in [0,1,2]:
        ...
        if self.game.three_red_letter.status == False and self.game.two_red_letter.status == False:
            if mix3 in [15,16]:
                if self.game.double_red.status == False:
                    ...
                    if self.game.triple.status == False:
                        return 1
                    elif self.check_mixer2(self.game.selection_feature.position) == True:
                        return 1
        else:
            ...
            if self.game.cu:
                if self.game.green_odds.position >= 3:
                    if self.game.mixer1.position in [2,21,17,4,23]:
                        if self.game.mixer2.position in [5,7,19,22]:
                            if self.game.triple.status == False:
                                self.game.triple.engage(self.game)
                                self.game.sound.play('tilt')
    ...
    def check_double_trips(self):
        ...
        m4 = self.game.mixer4.position
        m5 = self.game.mixer5.position
        if m1 in [1,2,3,4,5,7,8,9,11,12]:
            if self.game.blue_odds.position >= 5 and self.game.yellow_odds.position >= 5 and self.game.green_odds.position >= 5 and self.game.red_odds.position >= 5:
                if m5 == 0:
                    if m4 == 3:
                        if self.game.cu or self.game.triple.status == False:
                            if self.game.green_odds.position >= 3:
                                if m3 in [1,2,5,9,13,18,21,22]:
                                    if self.game.double_yellow.status == False:
                                        self.game.double_yellow.engage(self.game)
                                        self.game.sound.play('tilt')
                                elif m3 in [3,4,8,11,12,15,16,23]:
                                    if self.game.double_red.status == False:
                                        self.game.double_red.engage(self.game)
                                        self.game.sound.play('tilt')
                                elif m3 in [6,7,14,19]:
                                    if self.game.double_blue.status == False:
                                        self.game.double_blue.engage(self.game)
                                        self.game.sound.play('tilt')
                                elif m3 in [0,10,17,20]:
                                    if self.game.double_green.status == False:
                                        self.game.double_green.engage(self.game)
                                        self.game.sound.play('tilt')
            elif self.game.blue_odds.position <= 4 and self.game.yellow_odds.position <= 4 and self.game.green_odds.position <= 4 and self.game.red_odds.position <= 4:
                if m5 == 6:
                    if m4 == 3:
                         if self.game.cu or self.game.triple.status == False:
                            if self.game.green_odds.position >= 3:
                                if m3 in [1,2,5,9,13,18,21,22]:
                                    ...
                                    if self.game.double_green.status == False:
                                        self.game.double_green.engage(self.game)
                                        self.game.sound.play('tilt')
            elif self.game.blue_odds.position <= 3 and self.game.yellow_odds.position <= 3 and self.game.green_odds.position <= 3 and self.game.red_odds.position <= 3:
                if m5 in [13,1,4,7,10,18,19,22]:
                    if m4 == 3:
                         if self.game.cu or self.game.triple.status == False:
                            if self.game.green_odds.position >= 3:
                                if m3 in [1,2,5,9,13,18,21,22]:
                                    ...
                                    if self.game.double_green.status == False:
                                        self.game.double_green.engage(self.game)
                                        self.game.sound.play('tilt')
            elif self.game.blue_odds.position <= 2 and self.game.yellow_odds.position <= 2 and self.game.green_odds.position <= 2 and self.game.red_odds.position <= 2:
                if m5 in [13,1,2,3,5,8,9,14,15,20,21,23]:
                    if m4 == 3:
                         if self.game.cu or self.game.triple.status == False:
                            if self.game.green_odds.position >= 3:
                                if m3 in [1,2,5,9,13,18,21,22]:
            ...
            if self.game.odds_only.status == True:
                if self.game.blue_odds.position > 5 and self.game.yellow_odds.position > 5 and self.game.green_odds.position > 5 and self.game.red_odds.position > 5:
                                ...
                                if self.game.green_odds.position >= 3:
                                    if m3 in [1,2,5,9,13,18,21,22]:
                                        ...
                                        if self.game.double_green.status == False:
                                            self.game.double_green.engage(self.game)
                                            self.game.sound.play('tilt')
                elif self.game.blue_odds.position == 5 and self.game.yellow_odds.position == 5 and self.game.green_odds.position == 5 and self.game.red_odds.position == 5:
                    if m5 == [7,9,11,16]:
                        if m4 == 3:
                            if self.game.cu or self.game.triple.status == False:
                                if self.game.green_odds.position >= 3:
                    ...
                    if m5 in [0,3,4,5,9,10,12,19,23,18,20]:
                        if m4 == 3:
                             if self.game.cu or self.game.triple.status == False:
                                if self.game.green_odds.position >= 3:
                                    if m3 in [1,2,5,9,13,18,21,22]:
                    ...
                    if m5 in [9,16,17,1,18,20,21,22,0,3,4,5,9,10,12,13,14,17,19,23]:
                        if m4 == 3:
                             if self.game.cu or self.game.triple.status == False:
                                if self.game.green_odds.position >= 3:
                                    if m3 in [1,2,5,9,13,18,21,22]:
    ...
    def features_spotting(self):
        ...
        if self.game.cu or self.game.selection_feature.position < 9:
            ...
            if sd in [2,3,4]:
                if self.game.mystic_lines.position >= 7:
                    self.step_mystic_lines(10)
            if sd in [5,6,18,19,20,21,36,37,41,42,43,44]:
                self.step_twin(1)
            if sd in [12,13,14,25,38]:
                self.step_twin(4)

    def step_twin(self, number):
        if number >= 1:
            self.game.twin_number.step()
            if self.game.twin_number.position == 4:
                if self.game.three_red_letter.status == False:
                    self.game.eleven.engage(self.game)
                    self.game.sound.play('tilt')
                else:
                    self.game.seventeen.engage(self.game)
                    self.game.sound.play('tilt')
            if self.game.twin_number.position == 8:
                if self.game.eleven.status == False:
                    self.game.eleven.engage(self.game)
                    self.game.sound.play('tilt')
                else:
                    self.game.seventeen.engage(self.game)
            ...
            number -= 1
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
            self.delay(name="step_sc", delay=0.1, handler=self.step_twin, param=number)
    ...
    def animate_special(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.london.special_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
            self.delay(name="special_animation", delay=0.06, handler=self.animate_special, param=args)
        else:
            self.cancel_delayed(name="special_animation")
            self.delay(name="display", delay=0.1, handler=graphics.london.display, param=self)
            self.scan_special()
    ...
    def animate_odds_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.london.odds_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_line1(self, args):
        ...
        line = args[2]
        if num < 55:
            graphics.london.line1_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
    ...
    def animate_line2(self, args):
        ...
        line = args[2]
        if num < 55:
            graphics.london.line2_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
            args = [self.game,num,line]
    ...
    def animate_line3(self, args):
        ...
        line = args[2]
        if num < 55:
            graphics.london.line3_animation([self, num * -1, line])
            self.cancel_delayed(name="display")
            num = num + 1
            args = [self.game,num,line]
    ...
    def animate_features_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.london.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.london.both_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.london.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class London(procgame.game.BasicGame):
    """ London was the first game with the Twin Numbers feature """
    def __init__(self, machine_type):
        super(London, self).__init__(machine_type)
        ...
        #Odds steppers
        self.red_odds = units.Stepper("red_odds", 10, 'london')
        self.yellow_odds = units.Stepper("yellow_odds", 10, 'london')
        self.green_odds = units.Stepper("green_odds", 10, 'london')
        self.blue_odds = units.Stepper("blue_odds", 10, 'london')
        ...
        self.blue_replay_counter = units.Stepper("blue_replay_counter", 1800)
        ...
        self.twin_number = units.Stepper("twin_number", 8)

        self.line1 = units.Stepper("line1", 3, "london", "continuous")
        self.line2 = units.Stepper("line2", 1, "london", "continuous")
        self.line3 = units.Stepper("line3", 1, "london", "continuous")
        ...
        self.triple = units.Relay("triple")
        ...
        self.special_odds = units.Stepper("special_odds", 9, 'london')
        self.special_replay_counter = units.Stepper("special_replay_counter", 1800)
        self.special_game = units.Stepper("special_game", 3)
        self.special_color = units.Stepper("special_color", 14, "london", "continuous")
        self.special_reflex = units.Reflex("special_reflex", 200)
        self.missed = units.Relay("missed")

        self.white1 = units.Relay("white1")
        self.pink1 = units.Relay("pink1")
        self.orange1 = units.Relay("orange1")
        self.white2 = units.Relay("white2")
        self.pink2 = units.Relay("pink2")
        self.orange2 = units.Relay("orange2")

        self.triangle = units.Relay("triangle")
        self.square = units.Relay("square")
        self.circle = units.Relay("circle")

        self.eleven = units.Relay("eleven")
