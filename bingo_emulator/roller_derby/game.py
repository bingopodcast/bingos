...
from bingo_emulator.graphics.roller_derby import *
...
class SinglecardBingo(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        if self.game.start.status == True:
            ...
            self.game.features.disengage()
            self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.roller_derby.display, param=self)
    ...
    def sw_enter_active(self, sw):
        if self.game.switches.left.is_active() and self.game.switches.right.is_active():
            ...
            os.system("/home/nbaldridge/proc/bingo_emulator/start_game.sh roller_derby")
        else:
    ...
    def regular_play(self, red_letter=0):
        ...
        if self.game.start.status == True:
            ...
            graphics.roller_derby.display(self)
            self.check_lifter_status()
        else:
            if red_letter > 0:
                if self.game.switches.shutter.is_inactive():
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.roller_derby.reel1, graphics.roller_derby.reel10, graphics.roller_derby.reel100)
                self.game.coils.registerDown.pulse()
                number -= 1
                self.delay(name="display", delay=0, handler=graphics.roller_derby.display, param=self)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.roller_derby.reel1, graphics.roller_derby.reel10, graphics.roller_derby.reel100)
    ...
    def all_probability(self):
        ...
        if self.game.reflex.connected_rivet() == 1 and (mix1 in [2,7,11,14,16,20,22,24,6,5,13,15]):
            ...
            if self.game.green_odds.position > 0:
                self.scan_features()
        elif self.game.reflex.connected_rivet() == 2 and (mix1 in [2,7,11,14,16,20,22,24,26,5,13,15,4,9,23]):
            self.scan_odds()
            if self.game.green_odds.position > 0:
                self.scan_features()
    ...
    def animate_screen_right(self, args):
        ...
        num = args[1]
        if num >= 0:
            graphics.roller_derby.screen_animation([self, num, "right"])
    ...
    def animate_screen_left(self, args):
        ...
        num = args[1]
        if num >= 0:
            graphics.roller_derby.screen_animation([self, num, "left"])
            self.cancel_delayed(name="display")
            num = num - 1
    ...
    def animate_odds_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.roller_derby.odds_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_features_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.roller_derby.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_both(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.roller_derby.both_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_eb_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.roller_derby.eb_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class RollerDerby(procgame.game.BasicGame):
    """ Roller Derby was the first Magic Screen game with pic-a-play """
    def __init__(self, machine_type):
        super(RollerDerby, self).__init__(machine_type)
        ...
        self.cu = 1
        self.cam4 = units.Stepper("cam4", 3, "roller_derby", "continuous")
        ...
        #Odds steppers
        self.red_odds = units.Stepper("red_odds", 8, 'roller_derby')
        self.yellow_odds = units.Stepper("yellow_odds", 8, 'roller_derby')
        self.green_odds = units.Stepper("green_odds", 8, 'roller_derby')
...
game = RollerDerby(machine_type='pdb')
game.reset()
game.run_loop()
